...
(function (global) {
  ...
  async function saveCuppingAtlasEditor(opts) {
    if (!global.settings) return false;
    ...
    if (!committed || committed.ok === false) {
      if (!opts?.silent) global.notify?.('⚠️ تعذر حفظ خرائط الحجامة', 'danger');
      return false;
    }
    if (typeof global.applyCuppingAtlasConfig === 'function') global.applyCuppingAtlasConfig(cfg);
  ...
  async function resetCuppingAtlasEditor() {
    ...
    if (!await saveCuppingAtlasEditor({ silent: true })) return false;
  ...
  async function previewCuppingMapsPrint() {
    if (!await saveCuppingAtlasEditor()) return false;
