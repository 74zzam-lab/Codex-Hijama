...
(function (global) {
  ...
  const REGISTRY_KEY = '__tdw_device_registry__';
  let _heartbeatTimer = null;
  let _runtimeState = null;

  function getRegistered(doc) {
    const operational = global.DB?.get?.(REGISTRY_KEY, null) || _runtimeState;
    if (Array.isArray(operational?.registered)) return operational.registered;
    const seeded = Array.isArray(doc?.devices?.registered) ? doc.devices.registered.slice() : [];
    if (seeded.length) {
      Promise.resolve(global.DB?.set?.(REGISTRY_KEY, {
        schemaVersion: 1,
        registered: seeded,
        migratedFromSignedLicenseAt: new Date().toISOString(),
      })).catch(() => {});
    }
    return seeded;
  }

  async function persistRegistered(list) {
    const state = {
      schemaVersion: 1,
    ...
    _runtimeState = state;
    const result = await Promise.resolve(global.DB?.set?.(REGISTRY_KEY, state));
  ...
  async function resignDoc(doc) {
    // Device status is operational SQLite state, not a license entitlement.
    return doc;
  }
  ...
  async function touchDevice(doc, deviceUuid, patch) {
    ...
    const persisted = await persistRegistered(list);
    if (!persisted.ok) return persisted;
    return { ok: true, device: list[idx], doc };
  }
  ...
  async function registerDevice(options) {
    ...
    return { ok: true, device, doc, created: true };
  }
  ...
  async function requestEnrollment(options) {
    ...
    return { ok: true, device, doc, pending: true };
  }
  ...
  async function transferDevice(fromDeviceUuid, toDevice, options) {
    ...
    if (existing) {
      ...
      const persisted = await persistRegistered(list);
      if (!persisted.ok) return persisted;
      target = { ok: true, device: list[list.length - 1], doc };
    }
