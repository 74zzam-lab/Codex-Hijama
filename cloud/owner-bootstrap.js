...
(function (global) {
  ...
  function getBootstrapConfig(doc) {
    ...
    return {
      ...
      emails,
      consumed: !!cfg.consumed || !!state.tokenConsumedAt,
      consumedAt: cfg.consumedAt || state.tokenConsumedAt || null,
      method: cfg.method || state.method || null,
      claimedBy: cfg.claimedBy || state.claimedBy || null,
      expiresAt: cfg.expiresAt || null,
      issuedAt: cfg.issuedAt || null,
      ttlHours: Number.isFinite(ttlHours) && ttlHours > 0 ? ttlHours : null,
      emergencyRecoveryHash: cfg.emergencyRecoveryHash || state.emergencyRecoveryHash || null,
      recoverySalt: cfg.recoverySalt || state.recoverySalt || null
    };
  }
  ...
  async function tryConsumeBootstrap(method, claimedBy, options) {
    ...
    try {
      ...
      await Promise.resolve(global.DB?.set?.(BOOTSTRAP_STATE_KEY, {
        ...loadState(),
        tokenConsumedAt: now,
        claimedBy: claimedBy || null,
      ...
      return { ok: true, doc: snap, consumedAt: now };

      /* Retired mutation of the signed license document.
      ...
      if (global.OwnerHub?.saveLicenseDoc) {
        try { await global.OwnerHub.saveLicenseDoc(after); } catch {}
      } else if (typeof global.LicenseCloud?.pushToDrive === 'function') {
        try { await global.LicenseCloud.pushToDrive(after); } catch {}
      }
  ...
  async function redeemSetupToken(token, profileInput) {
    ...
    saveState({
      emergencyRecoveryHash: created.profile?.recovery?.hash || null,
    ...
    try {
      ...
      if (false && doc && created.profile?.recovery?.hash) {
        doc.ownerBootstrap = {
          ...(doc.ownerBootstrap || {}),
