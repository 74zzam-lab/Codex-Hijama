...
(function (global) {
  ...
  async function verifyPassword(username, password) {
    ...
    if (Number(profile.schemaVersion) >= 2 && profile.credentialUserId) {
      const rbac = global.cuppingElectron?.rbac || global.tadawi?.rbac;
      if (!rbac?.authenticateUser) return false;
      try {
        const authenticated = await rbac.authenticateUser({
          userId: profile.credentialUserId,
          role: 'owner',
          password: String(password || ''),
        });
        return authenticated?.ok === true;
      } catch {
        return false;
      }
    }
  ...
  async function rotatePassword(nextPassword, options) {
    ...
    if (Number(profile.schemaVersion) >= 2 && profile.credentialUserId) {
      const ownerUser = (global.users || []).find((user) => String(user?.id) === String(profile.credentialUserId));
      if (!ownerUser || !/^(?:pbkdf2:|pbkdf2v2:|b64:)/.test(String(ownerUser.password || ''))) {
        return { ok: false, error: 'authoritative_owner_credential_missing' };
      }
      profile.passwordHash = null;
      profile.credentialRevision = Number(ownerUser.credentialRevision) || profile.credentialRevision || 1;
    } else {
    ...
    profile.passwordChangedAt = nowIso();
    const epoch = bumpSessionEpoch(profile);
    const committed = await global.DB?.set?.(OWNER_PROFILE_KEY, profile);
    if (committed && committed.ok === false) {
      return { ok: false, error: committed.error || 'owner_profile_projection_commit_failed' };
    }
  ...
  async function emergencyRecoverOwner(input) {
    ...
    global.OwnerBootstrap?.saveState?.({
    ...
    const promoted = await global.OwnerMigration?.promoteUserToOwnerRole?.(username);
  ...
  async function transferOwnership(input) {
    ...
    const demoted = await global.OwnerMigration?.demoteOwnerRole?.(oldUsername, { toRole: input.demoteToRole || 'admin' });
    if (demoted?.ok === false) return demoted;
    ...
    const promoted = await global.OwnerMigration?.promoteUserToOwnerRole?.(newUsername, { noCurrentUserFallback: true });
    if (promoted?.ok === false) return promoted;
