...
(function (global) {
  ...
  async function persistPulledLicense(data, path, options) {
    ...
    if (typeof global.LicenseCloud?.saveLocalCommitted !== 'function') {
      return { ok: false, error: 'license_local_commit_unavailable' };
    }
    const committed = await global.LicenseCloud.saveLocalCommitted(data);
    if (!committed || committed.ok !== true) {
      return {
        ok: false,
        error: committed?.error || 'license_local_commit_failed',
        rolledBack: committed?.rolledBack === true,
        preservedLocal: true,
      };
    }
    return { ok: true, license: data, fromDrive: true, path: path || null, persisted: true };
  }
  ...
  async function downloadBranchConfiguration(branchId) {
    ...
    for (const fileName of files) {
      try {
        const r = await global.SyncEngine?.pullConfigFile?.(branchId, fileName, { preferRemote: true });
        const optionalMissing = fileName === 'owner.json' && ['not_found', 'no_file'].includes(String(r?.error || r?.reason || ''));
        results.push({
          file: fileName,
          ok: r?.ok === true || optionalMissing,
          optionalMissing,
          skipped: !!r?.skipped,
          error: r?.ok === true || optionalMissing ? null : (r?.error || r?.reason || 'pull_failed')
        });
      } catch (e) {
        results.push({ file: fileName, ok: false, error: e.message || String(e) });
      }
    }
    const okCount = results.filter(r => r.ok && !r.optionalMissing).length;
    const failed = results.filter(r => r.ok !== true);
    return { ok: failed.length === 0, branchId, results, downloaded: okCount, failed };
  }
  ...
  async function hydrateFromDrive(branchId, options) {
    ...
    const out = { ok: false, branchId, centerId, steps: [], analysis };
    ...
    out.ok = cfg?.ok === true && ops?.ok === true;
    if (!out.ok) {
      out.error = cfg?.ok !== true ? 'configuration_pull_incomplete' : 'operational_pull_incomplete';
    }
    ...
    if (out.ok && options.markComplete !== false) markBootstrapComplete(branchId);
