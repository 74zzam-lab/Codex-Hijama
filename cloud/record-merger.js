...
(function (global) {
  ...
  async function applyMergeToRepository(table, mergeResult, options) {
    options = options || {};
    if (!mergeResult?.merged) return { ok: false, error: 'no_merge_result' };
    if (!global.SyncedWrite?.setTable) return { ok: false, error: 'authoritative_write_unavailable' };
    const committed = await global.SyncedWrite.setTable(table, mergeResult.merged, {
      source: options.source || 'merge',
