...
(function (global) {
  ...
  async function executeSafeAuto(analysis, options) {
    ...
    // P0-D cutover: legacy snapshot analysis is display-only. All mutations
    // delegate to the one per-record immutable operation workflow.
    const operationSync = await global.SyncEngine?.runOnce?.({
      direction: 'both',
      force: options.force === true,
    }) || { ok: false, error: 'sync_engine_unavailable' };
    return {
      ok: operationSync.ok === true,
      protocol: 'tdw.sync.operation.v3',
      sync: operationSync,
      analysis,
    };

    /* Legacy snapshot mutation workflow intentionally disabled by P0-D.
