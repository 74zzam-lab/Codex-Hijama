...
(function (global) {
  ...
  function filterUsersForBranch(users, branchId) {
    ...
    return users.filter(u => {
      ...
      const copy = { ...u };
      // Passwords are already one-way PBKDF2 hashes. Keep verified hashes so
      // restored owners can sign in, but never upload legacy/plain credentials.
      const portableHash = /^(?:pbkdf2:|pbkdf2v2:|b64:)/i.test(String(copy.password || ''));
      if (!portableHash) delete copy.password;
      delete copy.legacyPassword;
