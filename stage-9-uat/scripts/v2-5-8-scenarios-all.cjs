...
async function main() {
  ...
  await scenario('A02-wizard-steps', 'Unified stepper Google→…→Ready', async () => {
    ...
    if (!v258 && !v259 && !v2510 && !v2511 && !v2512 && !v2513 && !v2514) throw new Error('NEW_STEPS mismatch');
    return {
      steps: v259 ? 8 : 9,
      version: v2514 ? 'v2-5.13-stage9' : (v2513 ? 'v2-5.12-stage8' : (v2512 ? 'v2-5.11-stage7' : (v2511 ? 'v2-5.11-stage6' : (v2510 ? 'v2-5.10' : (v259 ? 'v2-5.9' : 'v2-5.8')))))
    };
  });
