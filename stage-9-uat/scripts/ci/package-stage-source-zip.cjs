...
const EXCLUDE_DIRS = new Set([
  ...
  '01-ORIGINAL', '02-CURRENT', '03-REPORTS', 'stage-1-uat', 'stage-2-uat', 'stage-3-uat', 'stage-4-uat', 'stage-5-uat', 'stage-6-uat', 'stage-7-uat', 'stage-8-uat', 'stage-9-uat', 'app-source',
]);
...
const buildId = process.env.STAGE7_BUILD_ID || process.env.STAGE6_BUILD_ID || process.env.STAGE5_BUILD_ID || process.env.STAGE4_BUILD_ID || process.env.STAGE3_BUILD_ID || process.env.STAGE2_BUILD_ID || 'local';
const evidenceSubdir = stage === '7'
  ? 'STAGE-7-EXPLICIT-DISCOVERY'
  : (stage === '6'
    ? 'STAGE-6-ACTIVATION-BEFORE-GOOGLE'
    : (stage === '5'
      ? 'STAGE-5-BOOTSTRAP-GATE-MAP'
      : (stage === '4'
...
const required = ['package.json', 'cloud/boot-flow-ui.js', 'cloud/bootstrap-gates.js', 'cloud/post-google-cloud-discovery.js', 'tests/baseline/test-stage-7-explicit-discovery-gate.js'];
