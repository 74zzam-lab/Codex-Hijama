 ...
 * NEW: Language → Activation → Google → Discovery → Path decision (if needed) → Organization → Owner → Branch → Restore → Sync → Ready
...
(function (global) {
  ...
  const NEW_STEPS = ['language', 'license', 'google', 'discovery', 'path_decision', 'organization', 'owner', 'branch', 'restore', 'sync', 'ready'];
        ...
        : (version < 6
          ...
          : (version < 7
            ...
            : (version < 8
              ...
              : (version < 9 ? LEGACY_NEW_STEPS_PRE_STAGE9 : NEW_STEPS))));
  ...
  function validateStep(step) {
    switch (step) {
      ...
      case 'branch': {
        ...
        return hasBranch() && hasDeviceBranch();
      }
