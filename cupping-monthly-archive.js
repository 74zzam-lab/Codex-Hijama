...
(function (global) {
  ...
  async function deleteTemplate(id) {
    ...
    if (_activeTemplateId === id) {
      ...
      await applyTemplate('accounting');
    }
    if (!await saveArchiveSettings()) return false;
    renderTemplateSelect();
  ...
  async function onTemplateChange() {
    const id = document.getElementById('ma-template-sel')?.value;
    if (id) await applyTemplate(id);
  }

  async function registerReport(def) {
  ...
  async function openModal() {
    ...
    if (ma.templates[_activeTemplateId]) {
      await applyTemplate(_activeTemplateId);
    } else {
      _order = [...ma.order];
  ...
  async function moveReport(id, dir) {
    ...
    el.innerHTML = `
      <div class="modal" role="dialog">
        ...
        <div class="ma-template-bar">
          ...
          <button type="button" class="btn btn-ghost btn-sm" id="ma-template-delete" style="display:none" onclick="MonthlyArchive.deleteSelectedTemplate()">🗑️</button>
        </div>
        <div class="ma-period">
