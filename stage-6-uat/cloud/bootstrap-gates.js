...
(function (global) {
  ...
  function getStepInventory() {
    return [
      ...
      { stepId: 'license', displayName: 'التفعيل والترخيص', path: 'BOTH', currentPositionNew: 1, currentPositionExisting: 2, uiRenderer: 'renderStepUI:license', entry: 'activateLicenseKey', completionChecker: 'hasValidLicense()', sourceOfTruth: '__tdw_cloud_license__+LicenseActivationGate', writes: ['setupCommitSignedActivation', 'activation state'], cloudEffect: true, sqliteEffect: true, canRetry: true, canResume: true, canSkip: false, dependsOn: ['language'], produces: ['consumed activation'] },
      { stepId: 'google', displayName: 'ربط Google', path: 'BOTH', currentPositionNew: 2, currentPositionExisting: 1, uiRenderer: 'renderStepUI:google', entry: 'runGoogleConnect', completionChecker: 'hasGoogle()', sourceOfTruth: 'DriveAdapter+settings.backup.providers.google', writes: ['settings oauth', 'wizard path/completedSteps on discovery'], cloudEffect: true, sqliteEffect: true, canRetry: true, canResume: true, canSkip: false, dependsOn: ['license (NEW)', 'language (EXISTING)'], produces: ['google session', 'post-google cloud discovery'] },
