...
(() => {
  ...
  check(ctx.BootFlow.NEW_STEPS[resume] === 'google', 'restart after activation resumes at google');
})();
...
// 15. EXISTING unchanged — google before license
(() => {
  const steps = baseEnv().BootFlow.EXISTING_STEPS;
  check(steps[1] === 'google' && steps[2] === 'license', 'EXISTING google-first unchanged');
})();
...
// 20. runtime tail unchanged after google
(() => {
  const steps = baseEnv().BootFlow.NEW_STEPS;
  const tail = steps.slice(steps.indexOf('google') + 1);
  check(JSON.stringify(tail) === JSON.stringify(['organization', 'branch', 'restore', 'owner', 'sync', 'ready']), 'NEW tail after google unchanged');
})();
...
(() => {
  ...
  check(first && first.id === 'GOOGLE_CONNECTED', 'gate model: activation resolved → google next');
})();
