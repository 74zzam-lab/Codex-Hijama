...
function commitSetupBranchOnly(payload = {}) {
  ensureDb();
  const authority = readAuthorityIdentity();
  const centerId = String(authority.centerId || '').trim();
  ...
  if (!branchId) return { ok: false, error: 'setup_branch_required', rolledBack: true };
  const selectedBranch = (Array.isArray(license.branches) ? license.branches : [])
  ...
  try {
    ...
    db.transaction(() => {
      ...
      db.prepare(`
        INSERT INTO meta(key, value) VALUES('setupBranchCommittedAt', ?)
        ON CONFLICT(key) DO UPDATE SET value=excluded.value
      `).run(committedAt);
    })();
    return {
    ...
    return {
      ok: false,
      error: error.code || 'setup_branch_commit_failed',
      message: error.message,
      rolledBack: true,
    };
  }
