...
const SETUP_RESTORE_METADATA_TABLES = new Set([
  'schema_migrations', 'meta', 'sync_meta', 'p0b_migration_reports',
]);

const SETUP_RESTORE_BOOTSTRAP_ENTITY_TYPES = new Set([
  'settings', '__tdw_device_registry__',
]);

const SETUP_RESTORE_BOOTSTRAP_KV_KEYS = new Set([
  '__tdw_cloud_license__', '__tdw_meta__', '__tdw_device_config__',
  'commercial_license_data_v2', '__tdw_boot_wizard__', '__tdw_drive_folders__',
  '__tdw_repo_revisions__', '__tdw_versions__', '__tdw_sync_state__',
  '__tdw_branch_idempotency__', '__tdw_owner_migration__',
]);
...
function isSeedUser(value) {
  const user = value && typeof value === 'object' ? value : null;
  if (!user) return false;
  return user.seedDefaultPassword === true || user.mustChangePassword === true;
}

function isBootstrapOwnerMarker(value) {
  if (!value || typeof value !== 'object') return true;
  return String(value.status || '').toLowerCase() !== 'complete'
    && !value.ownerUserId
    && !value.credentialUserId;
}

/**
 * Classify the live target before a setup-only restore.
 *
 * Setup metadata, verified activation records, device enrollment rows and only
 * seeded/forced-change users may be replaced. Any real credential, business
 * row, quarantine row, conflict or unknown persisted row closes the gate.
 */
function classifySetupRestoreTarget(databasePath) {
  const dbPath = path.resolve(databasePath || '');
  ...
  try {
    ...
    const reasons = [];
    let hasMeaningfulUser = false;

    const mark = (table, bootstrap, meaningful, reason) => {
      tables[table] = { bootstrap, meaningful };
      bootstrapRows += bootstrap;
      meaningfulRows += meaningful;
      if (meaningful && reason) reasons.push(reason);
    };

    // Users exist in both the legacy normalized table and P0-B entity storage.
    if (tableNames.includes('users')) {
      const rows = db.prepare('SELECT payload_json FROM users').all();
      let seeded = 0;
      let real = 0;
      for (const row of rows) (isSeedUser(parseStoredJson(row.payload_json)) ? seeded++ : real++);
      hasMeaningfulUser ||= real > 0;
      mark('users', seeded, real, real ? 'usable_or_nonseed_user' : null);
    }
    if (tableNames.includes('p0b_entities')) {
      const rows = db.prepare('SELECT entity_type, payload_json, deleted_at FROM p0b_entities').all();
      let bootstrap = 0;
      let meaningful = 0;
      for (const row of rows) {
        const isUser = String(row.entity_type) === 'users';
        const seed = isUser && !row.deleted_at && isSeedUser(parseStoredJson(row.payload_json));
        const setupEntity = !row.deleted_at && SETUP_RESTORE_BOOTSTRAP_ENTITY_TYPES.has(String(row.entity_type));
        if (seed || setupEntity) bootstrap += 1;
        else meaningful += 1;
      }
      hasMeaningfulUser ||= rows.some((row) => String(row.entity_type) === 'users'
        && !row.deleted_at && !isSeedUser(parseStoredJson(row.payload_json)));
      mark('p0b_entities', bootstrap, meaningful, meaningful ? 'operational_p0b_entities' : null);
    }

    for (const table of tableNames) {
      if (tables[table]) continue;
      const count = Number(db.prepare(`SELECT COUNT(*) AS c FROM "${table.replace(/"/g, '""')}"`).get()?.c || 0);
      if (!count) {
        tables[table] = { bootstrap: 0, meaningful: 0 };
        continue;
      }
      if (SETUP_RESTORE_METADATA_TABLES.has(table)) {
        mark(table, count, 0);
        continue;
      }
      if (table === 'kv_store') {
        ...
        for (const row of db.prepare('SELECT key, value_json FROM kv_store').all()) {
          ...
          const value = parseStoredJson(row.value_json);
          const allowed = SETUP_RESTORE_BOOTSTRAP_KV_KEYS.has(key)
            || (key === '__tdw_owner_setup__' && isBootstrapOwnerMarker(value));
          if (allowed) bootstrap += 1;
          else meaningful += 1;
        }
        mark(table, bootstrap, meaningful, meaningful ? 'operational_or_unknown_kv' : null);
        continue;
      }
      if (table === 'device_registry_local') {
        mark(table, count, 0);
        continue;
      }
      // The organization step persists exactly one settings singleton before
      // the restore step. It is setup metadata until a real user or business
      // row exists, and must not make the documented setup restore impossible.
      if (table === 'settings' && count === 1 && !hasMeaningfulUser) {
        mark(table, count, 0);
        continue;
      }
      if (table === 'p0b_commands') {
        const nonBootstrap = Number(db.prepare(`
          SELECT COUNT(*) AS c FROM p0b_commands
          WHERE entity_type NOT IN ('users', 'settings', '__tdw_device_registry__')
        `).get()?.c || 0);
        const bootstrap = !hasMeaningfulUser ? count - nonBootstrap : 0;
        mark(table, bootstrap, count - bootstrap, count - bootstrap ? 'committed_operational_commands' : null);
        continue;
      }
      if (table === 'sync_outbox') {
        const nonBootstrap = Number(db.prepare(`
          SELECT COUNT(*) AS c FROM sync_outbox
          WHERE table_name NOT IN ('users', 'settings', '__tdw_device_registry__')
        `).get()?.c || 0);
        const bootstrap = !hasMeaningfulUser ? count - nonBootstrap : 0;
        mark(table, bootstrap, count - bootstrap, count - bootstrap ? 'pending_or_historical_operational_sync' : null);
        continue;
      }
      mark(table, 0, count, `persisted_rows:${table}`);
    }

    const classification = meaningfulRows > 0
      ? 'populated'
      : (bootstrapRows > 0 ? 'bootstrap_only' : 'empty');
    return {
      ok: true,
      classification,
      replaceAllowed: meaningfulRows === 0,
      bootstrapRows,
      meaningfulRows,
    ...
    return {
      ...
      bootstrapRows: 0,
      meaningfulRows: 0,
      reasons: [error.code || error.message || 'classification_failed'],
      tables: {},
    };
  } finally {
    try { db?.close(); } catch { /* ignore */ }
...
async function restoreBackupFile(options) {
  ...
  try {
    ...
    const stagedDatabasePath = path.join(stageRoot, DATABASE_PATH.replace(/\//g, path.sep));
    const migration = migrateStagedDatabase(stagedDatabasePath, options.now || new Date());
    if (typeof options.prepareStagedDatabase === 'function') {
      await options.prepareStagedDatabase(stagedDatabasePath, inspected.manifest);
      const preparedHealth = databaseHealth(stagedDatabasePath);
      if (!preparedHealth?.ok) throw new Error('prepared_staged_sqlite_integrity_failed');
    }
