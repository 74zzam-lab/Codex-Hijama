...
class BackupV2Scheduler {
  ...
  normalize(input) {
    ...
    return {
      ...
      centerId: typeof config.centerId === 'string' ? config.centerId.slice(0, 128) : '',
      organizationId: typeof config.organizationId === 'string' ? config.organizationId.slice(0, 128) : '',
      branchId: typeof config.branchId === 'string' ? config.branchId.slice(0, 128) : '',
      authorizedBranchIds: Array.isArray(config.authorizedBranchIds)
        ? config.authorizedBranchIds.map((value) => String(value).slice(0, 128)).filter(Boolean).slice(0, 100)
        : [],
      deviceId: typeof config.deviceId === 'string' ? config.deviceId.slice(0, 128) : '',
