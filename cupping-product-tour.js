...
(function (global) {
  ...
  async function saveFirstRunState(patch) {
    if (typeof global.ensureFirstRunSettings === 'function') global.ensureFirstRunSettings();
    if (!global.settings?.firstRun) return false;
...
.fr-tour-actions{display:flex;gap:8px;flex-wrap:wrap}`;
  ...
  async function onTourNext() {
    if (_tourStep >= TOUR_STEPS.length - 1) return completeProductTour();
    return goTourStep(_tourStep + 1);
  }

  async function goTourStep(n) {
    _tourStep = Math.max(0, Math.min(TOUR_STEPS.length - 1, n));
    if (!await saveFirstRunState({ tourStep: _tourStep })) return false;
    positionTour();
  }

  async function skipProductTour() {
    if (!await saveFirstRunState({ tourSkipped: true, tourCompleted: false })) return false;
  ...
  async function completeProductTour() {
    if (!await saveFirstRunState({ tourCompleted: true, tourSkipped: false, tourStep: TOUR_STEPS.length - 1 })) return false;
  ...
  async function restartProductTour() {
    ...
    if (!await saveFirstRunState({ tourCompleted: false, tourSkipped: false, tourStep: 0 })) return false;
