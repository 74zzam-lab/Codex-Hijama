...
(function (global) {
  ...
  function findDuplicateClientGroups() {
    ...
    (global.clientsRegistry || []).forEach(c => {
      ...
      const centerId = c.centerId || c.center_id || global.CenterId?.getStoredCenterId?.() || '';
      const branchId = c.branchId || c.branch_id || global.BranchContexts?.getOperationalWriteBranch?.() || '';
      const scopedPhone = `${centerId}:${branchId}:${phone}`;
      if (!map[scopedPhone]) map[scopedPhone] = [];
      map[scopedPhone].push(c);
    });
  ...
  async function mergeDuplicateClients() {
    ...
    if (!global.SyncedWrite?.setTable) {
      notify('⛔ مسار الحفظ الآمن غير متاح', 'danger');
      return;
    }
    let merged = 0;
    let nextClients = JSON.parse(JSON.stringify(global.clientsRegistry || []));
    let nextCases = JSON.parse(JSON.stringify(global.cases || []));
    let nextSessions = JSON.parse(JSON.stringify(global.nextSessions || []));
    const groupIds = groups.map((group) => new Set(group.map((client) => String(client.id))));
    groups.forEach((sourceGroup, groupIndex) => {
      const group = nextClients.filter((client) => groupIds[groupIndex].has(String(client.id)));
      group.sort((a, b) => {
        const fa = a.fileNo || '';
        const fb = b.fileNo || '';
      ...
      nextClients = nextClients.filter(c => !dupIds.has(c.id));
      nextCases.forEach(cs => {
        group.slice(1).forEach(dup => {
    ...
    for (const [table, value] of [
      ['clientsRegistry', nextClients],
      ['cases', nextCases],
      ['nextSessions', nextSessions],
    ]) {
      const result = await global.SyncedWrite.setTable(table, value, { source: 'duplicate_merge' });
      if (!result?.ok) {
        notify(`⛔ فشل حفظ الدمج (${table}): ${result?.error || 'commit_failed'}`, 'danger');
        return;
      }
    }
  ...
  async function clearAllBusinessData() {
    if (!global.SyncedWrite?.wipeTable) throw new Error('authoritative_wipe_unavailable');
    for (const key of DATA_KEYS) {
      const result = await global.SyncedWrite.wipeTable(key);
      if (!result?.ok) throw new Error(`${key}:${result?.error || 'wipe_failed'}`);
    }
  }

  async function reseedDefaults() {
    if (!global.SyncedWrite?.setTable) throw new Error('authoritative_write_unavailable');
    ...
    let result = await global.SyncedWrite.setTable('settings', freshSettings, { source: 'factory_reset' });
    if (!result?.ok) throw new Error(`settings:${result?.error || 'reset_failed'}`);

    const freshUsers = JSON.parse(JSON.stringify(global.defaultUsers || []));
    result = await global.SyncedWrite.setTable('users', freshUsers, { source: 'factory_reset' });
    if (!result?.ok) throw new Error(`users:${result?.error || 'reset_failed'}`);

    for (const k of DATA_KEYS) {
      if (k === 'users' || k === 'settings') continue;
      const empty = k.includes('Counter') ? 1 : [];
      result = await global.SyncedWrite.setTable(k, empty, { source: 'factory_reset' });
      if (!result?.ok) throw new Error(`${k}:${result?.error || 'reset_failed'}`);
    }
  ...
  async function applyPartialResetSections(sections) {
    ...
    if (!global.SyncedWrite?.wipeTable) {
      global.SyncGuard?.resume?.({ state: 'local_only' });
      throw new Error('authoritative_wipe_unavailable');
    }
    for (const k of keysToClear) {
      const isCounter = k.includes('Counter');
      const val = isCounter ? 1 : [];
      const result = await global.SyncedWrite.wipeTable(k, val);
      if (!result?.ok) {
        global.SyncGuard?.resume?.({ state: 'local_only' });
        throw new Error(`${k}:${result?.error || 'wipe_failed'}`);
      }
  ...
  async function executePartialReset() {
    ...
    await runWithGlobalLoading('جاري إعادة الضبط الجزئي...', async () => {
      ...
      try {
        await applyPartialResetSections(sections);
      } catch (error) {
        notify(`⛔ فشل إعادة الضبط: ${error?.message || error}`, 'danger');
        return;
      }
  ...
  async function executeFactoryReset() {
    ...
    await runWithGlobalLoading('جارٍ إنشاء نسخة طوارئ ومسح البيانات...', async () => {
      ...
      try {
        ...
        notify(`⛔ فشل مسح البيانات: ${error?.message || error}`, 'danger');
        return;
      }
