...
async function setPosBank(bankId) {
  settings.posBank = bankId;
  settings.bankProvider = bankId;
  const committed = await window.persistData?.('settings', settings);
  if (!committed || committed.ok === false) throw new Error(committed?.error || 'pos_bank_settings_commit_failed');
}
...
async function applyBankDefaultRates(bankId) {
...
async function applyRajhiDefaultRates() { return applyBankDefaultRates('rajhi'); }

async function onPosBankChange() {
