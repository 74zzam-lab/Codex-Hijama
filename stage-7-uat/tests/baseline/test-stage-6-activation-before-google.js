...
(() => {
  ...
  check(w.wizardFlowVersion === 7 || w.wizardFlowVersion === 6, 'legacy wizard migrated to flow v6+');
...
(() => {
  ...
  check(JSON.stringify(tail) === JSON.stringify(['organization', 'branch', 'restore', 'owner', 'sync', 'ready']), 'NEW tail after discovery unchanged');
})();
