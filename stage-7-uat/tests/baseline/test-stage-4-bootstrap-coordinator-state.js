...
function readyEnv(overrides = {}) {
  ...
  const snap = {
    ...
    wizard: {
      path: 'new', currentStep: 0, restoreChoice: 'empty', syncDone: false, completedSteps: [], wizardFlowVersion: 7,
...
(() => {
  const ctx = readyEnv({
    ...
    wizard: {
      ...
      syncDone: true, restoreChoice: 'empty', wizardFlowVersion: 7,
    },
  });
...
(() => {
  const ctx = readyEnv({
    ...
    wizard: {
      ...
      restoreChoice: 'empty', wizardFlowVersion: 7,
      discoveryCompletedAt: new Date().toISOString(),
      licenseDiscoveryAttempted: true,
      cloudDiscovery: { result: { ok: true, status: 'no_existing_business' }, googleAccountKey: null },
    },
