...
check(/authenticateUser/.test(indexSrc) && /_pendingRbacProof/.test(indexSrc), 'RBAC authenticates in main before proof-bound session');
check(!/seedUsersIfEmpty/.test(indexSrc), 'renderer cannot seed authoritative users before bind');
...
check(/commitLicenseRevision/.test(fs.readFileSync(path.join(root, 'cloud/branch-enrollment.js'), 'utf8'))
  && /requiresAdminReissue/.test(fs.readFileSync(path.join(root, 'cloud/branch-enrollment.js'), 'utf8')),
  'legacy branch revision is atomic and V6 still requires issuer reissue');
...
const sandbox = {
  ...
  DB: { get: () => null, set() { return { ok: true }; } },
  async persistData() { return { ok: true }; },
  CloudMeta: { isCloudV2Enabled: () => false, setCloudV2Enabled() {}, loadMeta: () => ({}), saveMeta() {} },
  CloudV2: { maybeAutoEnableCloudV2: async () => { sandbox.settings.cloudV2Enabled = true; return { ok: true }; } },
  ...
  SyncEngine: { isRunning: () => false, async start() { sandbox._started = true; return { ok: true }; } },
  SyncGuard: { resume: async () => ({ ok: true }) },
  BackupLayer: { start: async () => ({ ok: true }) },
  cuppingElectron: { backup: { v2ScheduleConfigure: async () => ({ ok: true }) } },
  SyncState: { load: () => ({}) },
...
const applied = await sandbox.ActivationSyncDefaults.applyDefaults({ startSync: true });
...
runRuntimeChecks().catch((error) => {
  console.error('FAIL: v2-5.9 final activation', error);
  process.exit(1);
});
