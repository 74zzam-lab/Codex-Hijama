 ...
 * NEW: Language → Activation → Google → Discovery → Organization → Branch → Restore → Owner → Sync → Ready
...
(function (global) {
  ...
  const NEW_STEPS = ['language', 'license', 'google', 'discovery', 'organization', 'branch', 'restore', 'owner', 'sync', 'ready'];
  const EXISTING_STEPS = ['language', 'google', 'discovery', 'license', 'organization', 'branch_select', 'restore', 'owner', 'sync', 'ready'];
  ...
  function isCriticalOpInFlight() {
    if (oauthInFlight || licenseActivateInFlight || branchCreateInFlight || branchBindInFlight || ownerLoginInFlight || restoreInFlight || syncInFlight || discoveryInFlight) {
      return true;
    }
  ...
  function normalizeWizardFlowState(w) {
    ...
    if (version < WIZARD_FLOW_VERSION) {
      const legacySteps = w.path === PATHS.EXISTING
        ? (version < 7 ? LEGACY_EXISTING_STEPS_PRE_STAGE7 : EXISTING_STEPS)
        : (version < 6 ? LEGACY_NEW_STEPS_PRE_STAGE6 : LEGACY_NEW_STEPS_PRE_STAGE7);
  ...
  function renderStepUI(w) {
    ...
    switch (step) {
      ...
      case 'discovery': {
        ...
        addBtn(actions, discoveryInFlight ? '⏳ جارٍ الاكتشاف...' : '🔍 تشغيل الاكتشاف', 'btn-primary', async () => {
    ...
    : (isNew
      ...
      case 'restore': {
        ...
        const runDiscovery = async () => {
          ...
          if (cachedDiscovery?.dataDiscovery) {
