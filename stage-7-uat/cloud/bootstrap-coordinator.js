...
(function (global) {
  ...
  const FIELD_AUTHORITY = Object.freeze({
    ...
    selectedCandidateId: 'KEEP_TEMPORARILY',
  ...
  function coordinatorSnapshot() {
    ...
    return {
      ...
      selectedCandidateId: raw.selectedCandidateId || raw.discoveryCandidateId || null,
  ...
  function isStepResolved(step, coord) {
    ...
    switch (step) {
      ...
      case 'branch':
      case 'branch_select':
        return !!(BF?.hasBranch?.() && BF?.hasDeviceBranch?.());
      case 'owner':
        return !!(BF?.ownerSetupRequirementMet?.() || SS?.hasOwnerCredential?.());
      case 'restore':
