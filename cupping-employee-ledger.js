...
(function (global) {
  ...
  async function saveStore() {
    const writes = [
      ['employeeLedgerAccruals', accruals],
      ['employeeLedgerPayments', payments],
      ['employeeLedgerEntries', entries],
      ['settings', global.settings],
    ];
    for (const [key, value] of writes) {
      const committed = await global.persistData?.(key, value);
      if (!committed || committed.ok === false) {
        throw new Error(committed?.error || `ledger_commit_failed:${key}`);
      }
    }
  ...
  async function registerAccrualType(def) {
  ...
  function isMonthClosed(month, year) {
    const authoritativeRun = (global.DB?.get?.('payrollRuns', []) || []).find((run) => (
      run && run.periodKey === `${year}-${String(month).padStart(2, '0')}` && run.status === 'finalized'
    ));
    if (authoritativeRun) return true;
    ensureLedgerSettings();
  ...
  async function closeMonth(month, year) {
    ...
    const payrollRows = visibleDoctors().map((doctor) => {
      const summary = getSummary(doctor.id, month, year);
      return {
        employeeId: doctor.id,
        employeeName: doctor.name,
        gross: summary.earned,
        deductions: summary.deducted,
        net: summary.due,
      };
    });
    const finalizeApi = (global.cuppingElectron || global.tadawi)?.database?.finalizePayrollRun;
    if (typeof finalizeApi !== 'function') {
      notify('تعذّر الوصول إلى مسار إقفال الرواتب الآمن — لم يتم الإقفال.', 'danger');
      return false;
    }
    const finalized = await finalizeApi({
      runId: `PAYROLL:${period}:${global.crypto?.randomUUID?.() || Date.now()}`,
      periodKey: period,
      rows: payrollRows,
    });
    if (!finalized?.ok) {
      notify(`تعذّر إقفال الرواتب بأمان: ${finalized?.error || 'payroll_finalize_failed'}`, 'danger');
      return false;
    }
  ...
  async function reopenMonth(month, year) {
    ...
    ensureLedgerSettings();
    const key = periodKey(year, month);
    const finalizedRun = (global.DB?.get?.('payrollRuns', []) || []).find((run) => (
      run && run.periodKey === `${year}-${String(month).padStart(2, '0')}` && run.status === 'finalized'
    ));
    if (finalizedRun) {
      notify('🔒 مسير الرواتب النهائي غير قابل لإعادة الفتح؛ استخدم تسوية صريحة مرتبطة بالمسير.', 'danger');
      return false;
    }
  ...
  async function createAdjustment(doctorId, type, amount, reason, month, year) {
    if (!hasPerm('ledger.adjust')) { notify('⛔ لا صلاحية للتسوية', 'danger'); return null; }
    const finalizedRun = (global.DB?.get?.('payrollRuns', []) || []).find((run) => (
      run && run.periodKey === `${year}-${String(month).padStart(2, '0')}` && run.status === 'finalized'
    ));
    if (!finalizedRun && !canEditPeriod(month, year)) { notify('🔒 الشهر مقفول', 'danger'); return null; }
    const doc = (global.doctors || []).find(d => d.id === doctorId);
    if (!doc) return null;
    ...
    if (finalizedRun) {
      const adjustApi = (global.cuppingElectron || global.tadawi)?.database?.adjustFinalizedPayroll;
      if (typeof adjustApi !== 'function') {
        notify('تعذّر الوصول إلى مسار تسوية الرواتب الآمن.', 'danger');
        return null;
      }
      const adjusted = await adjustApi({
        adjustmentId: `PAYADJ:${global.crypto?.randomUUID?.() || Date.now()}`,
        runId: finalizedRun.runId || finalizedRun.id,
        employeeId: doctorId,
        amount: isDed ? -amt : amt,
        reason: reason || 'تسوية صريحة بعد الإقفال',
      });
      if (!adjusted?.ok) {
        notify(`تعذّر تسجيل التسوية: ${adjusted?.error || 'payroll_adjustment_failed'}`, 'danger');
        return null;
      }
    }
  ...
  async function syncMonth(month, year, options) {
  ...
  async function syncMonthAndRender(month, year, options) {
    const result = await syncMonth(month, year, options);
    renderLedgerPage();
    return result;
  }

  async function closeMonthAndRender(month, year) {
    const result = await closeMonth(month, year);
    renderLedgerPage();
    return result;
  }

  async function reopenMonthAndRender(month, year) {
    const result = await reopenMonth(month, year);
    renderLedgerPage();
    return result;
  }
  ...
  function queueSyncForDate(doctorId, dateStr) {
    ...
    _syncTimer = setTimeout(async () => {
      ...
      if (doc) {
        try { loadStore(); syncDoctorMonth(doc, month, year); await saveStore(); }
        catch (error) { notify('⚠️ تعذر حفظ مزامنة مستحقات الموظف', 'danger'); }
      }
    }, 800);
  }
  ...
  async function saveAutoPrintSetting(enabled) {
    ensureLedgerSettings();
...
`;
  ...
  async function extRestoreData(data) {
    if (data.employeeLedgerAccruals) accruals = data.employeeLedgerAccruals;
    if (data.employeeLedgerPayments) payments = data.employeeLedgerPayments;
    if (data.employeeLedgerEntries) entries = data.employeeLedgerEntries;
    if (data.employeeLedgerAccruals || data.employeeLedgerPayments || data.employeeLedgerEntries) await saveStore();
    loadStore();
  }

  async function onPayrollGenerated(month, year) {
    if (!isModuleEnabled()) return;
    ensureLedgerSettings();
    if (global.settings.employeeLedger.autoSyncOnPayroll) await syncMonth(month, year, { carryOver: true });
  ...
  global.EmployeeLedger = {
    init: extInit, isModuleEnabled, syncMonth, syncMonthAndRender, closeMonthAndRender, reopenMonthAndRender,
    carryOverToMonth, getSummary, getDashboardStats, recordPayment,
