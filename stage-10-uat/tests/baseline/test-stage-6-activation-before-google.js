...
(() => {
  ...
  check(w.wizardFlowVersion === 9 || w.wizardFlowVersion === 8 || w.wizardFlowVersion === 7 || w.wizardFlowVersion === 6, 'legacy wizard migrated to flow v6+');
...
(() => {
  ...
  const expectedTail = steps.includes('owner') && steps.indexOf('owner') < steps.indexOf('branch')
    ? ['path_decision', 'organization', 'owner', 'branch', 'restore', 'sync', 'ready']
    : (steps.includes('path_decision')
      ? ['path_decision', 'organization', 'branch', 'restore', 'owner', 'sync', 'ready']
      : ['organization', 'branch', 'restore', 'owner', 'sync', 'ready']);
  check(JSON.stringify(tail) === JSON.stringify(expectedTail), 'NEW tail after discovery reflects Stage 9 owner-before-branch when present');
})();
