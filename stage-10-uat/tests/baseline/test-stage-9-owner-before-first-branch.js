...
// 1. NEW Start New order org→owner→branch
check(orgIdx >= 0 && ownerIdx > orgIdx && branchIdx > ownerIdx && restoreIdx > branchIdx,
  'NEW runtime order organization→owner→branch→restore');
...
(() => {
  ...
  check(w.wizardFlowVersion === 9, 'legacy v8 migrates to v9');
...
(() => {
  ...
  ]), 'direct EXISTING runtime unchanged');
})();
