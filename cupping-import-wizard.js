...
async function persistImportBatch() {
  ...
  const casesCommit = await SyncedWrite.setTable('cases', cases);
  const clientsCommit = await SyncedWrite.setTable('clientsRegistry', clientsRegistry);
  const counterCommit = await SyncedWrite.setTable('invoiceCounter', invoiceCounter);
  const logsCommit = await SyncedWrite.setTable('systemLogs', systemLogs);
  if (![casesCommit, clientsCommit, counterCommit, logsCommit].every((result) => result?.ok === true)) {
    throw new Error('import_persistence_failed');
  }
}
...
async function backupBeforeImport() {
  ...
  try {
    ...
    const committed = await window.SqliteBridge?.commitKv?.('preImportBackup', { at: new Date().toISOString(), snapshot: data });
    if (!committed || committed.ok === false) throw new Error(committed?.error || 'pre_import_backup_commit_failed');
  ...
  try {
    ...
    if (!_importCancelRequested) {
      ...
      if (fileHash) {
        ...
        const committed = await SyncedWrite.setTable('importHistory', history);
        if (!committed || committed.ok === false) throw new Error(committed?.error || 'import_history_commit_failed');
      }
