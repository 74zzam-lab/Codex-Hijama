...
function loadPrivateKey(keyPath) {
  if (!keyPath) {
    throw new Error('Private signing key path is required. Set TADAWI_LICENSE_PRIVATE_KEY or pass --private.');
  }
...
function issueLicense(input, privateKey) {
  ...
  const body = {
    ...
    keyId: input.keyId || 'prod-ed25519-2026-a7f929f51598',
  };
  for (const key of ['centerId', 'centerName', 'ownerIdentity', 'branches', 'activation', 'devices', 'licenseVersion']) {
...
function defaultKeyPaths(root) {
  ...
  return {
    // Never fall back to a repository-owned private key. Signing is an
    // explicit offline-admin operation and must name its protected key.
    privateKeyPath: process.env.TADAWI_LICENSE_PRIVATE_KEY || null,
