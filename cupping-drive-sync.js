...
async function commitDriveSyncSettings() {
  const writer = window.SyncedWrite?.setTable;
  if (typeof writer !== 'function') throw new Error('authoritative_write_unavailable');
  const result = await writer.call(window.SyncedWrite, 'settings', settings);
  if (!result?.ok) throw new Error(result?.error || 'drive_sync_settings_commit_failed');
  return result;
}
...
async function stopLegacyDriveSync(reason) {
...
async function runDriveSyncUpload(trigger) {
  if (isLegacyDriveSyncBlocked()) {
    await stopLegacyDriveSync('Cloud V2 active');
...
function startDriveSyncTimers() {
  ...
  clearInterval(_driveSyncDownloadTimer);
  if (isLegacyDriveSyncBlocked()) {
    stopLegacyDriveSync('Cloud V2 active').catch((error) => {
      console.error('[drive-sync] failed to persist legacy shutdown:', error);
    });
    return;
  }
  ensureDriveSyncSettings();
...
async function saveDriveSyncSettingsFromUI() {
  ensureDriveSyncSettings();
  const s = settings.driveSync;
...
async function saveAltSyncSettingsFromUI() {
  ensureDriveSyncSettings();
...
async function initDriveSyncUI() {
  if (isLegacyDriveSyncBlocked()) {
    await stopLegacyDriveSync('Cloud V2 active');
    renderDriveSyncUI();
    return;
