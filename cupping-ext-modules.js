...
// ── INVENTORY ──
async function seedHijamaInventory() {
  ...
  await persistData('inventoryItems', inventoryItems);
}
...
async function adjustInventoryStock(itemId, deltaPieces, reason, refId) {
  ...
  await persistData('inventoryItems', inventoryItems);
  await persistData('inventoryMovements', inventoryMovements);
  return true;
}

async function deductInventoryForCase(c) {
  ensureExtSettings();
  if (!settings.inventory.enabled || !settings.inventory.autoDeductOnSession) return;
  const cups = parseFloat(c.cups) || 0;
  if (!cups) return;
  for (const item of inventoryItems.filter(i => i.active && (i.perSession || 0) > 0)) {
    const qty = (item.perSession || 0) * cups;
    if (qty > 0) {
      await adjustInventoryStock(item.id, -qty, 'استهلاك جلسة', c.id);
      logSystem('inventory', 'خصم مخزون', `${item.name}: -${fmtNum(qty, 2)} (${c.name})`, { caseId: c.id });
    }
  }
}

/** Build (without persisting) the stock and cash projections committed by Main with a case. */
function buildAtomicCaseEffects(c) {
  ensureExtSettings();
  const effects = [];
  const projection = { inventoryItems: [], inventoryMovements: [], cashDrawerSession: null };
  const now = new Date().toISOString();
  const cups = Math.max(0, Number(c?.cups) || 0);
  if (settings.inventory?.enabled && settings.inventory?.autoDeductOnSession && cups > 0) {
    for (const item of inventoryItems.filter((row) => row.active && Number(row.perSession) > 0)) {
      const quantity = Number(item.perSession) * cups;
      const updated = { ...item, stockPieces: Math.max(0, Number(item.stockPieces || 0) - quantity) };
      const movement = {
        id: `CASE:${c.id}:ITEM:${item.id}`, at: now, itemId: item.id, itemName: item.name,
        delta: -quantity, balance: updated.stockPieces, reason: 'case_consumption', refId: c.id,
        user: currentUser ? currentUser.fullName : '—',
      };
      projection.inventoryItems.push(updated);
      projection.inventoryMovements.push(movement);
    }
    if (projection.inventoryItems.length) effects.push({ entity: 'inventoryItems', records: projection.inventoryItems });
    if (projection.inventoryMovements.length) effects.push({ entity: 'inventoryMovements', records: projection.inventoryMovements });
  }
  if (settings.cashFloat?.enabled) {
    const current = getTodayCashSession();
    const session = JSON.parse(JSON.stringify(current));
    session.id = '__singleton__';
    session.movements = Array.isArray(session.movements) ? session.movements : [];
    if (!session.foreign) session.foreign = { USD: 0, EUR: 0, GBP: 0 };
    if (Number(c.foreignAmount) > 0 && c.payCurrency && c.payCurrency !== 'SAR') {
      session.foreign[c.payCurrency] = Number(session.foreign[c.payCurrency] || 0) + Number(c.foreignAmount);
    }
    const cash = Number(c.cash) || 0;
    const change = Number(c.changeReturned) || 0;
    if (cash > 0) session.movements.unshift({
      id: `CASE:${c.id}:CASH`, at: now, amount: cash, reason: 'invoice_cash_receipt',
      meta: { caseId: c.id, type: 'in' }, user: currentUser ? currentUser.fullName : '—',
    });
    if (change > 0) session.movements.unshift({
      id: `CASE:${c.id}:CHANGE`, at: now, amount: -change, reason: 'customer_change',
      meta: { caseId: c.id, type: 'change' }, user: currentUser ? currentUser.fullName : '—',
    });
    projection.cashDrawerSession = session;
    effects.push({ entity: 'cashDrawerSession', records: [session] });
  }
  return { effects, projection };
}

function applyAtomicCaseProjection(projection) {
  if (!projection) return;
  for (const updated of projection.inventoryItems || []) {
    const index = inventoryItems.findIndex((row) => String(row.id) === String(updated.id));
    if (index >= 0) inventoryItems[index] = updated;
  }
  for (const movement of projection.inventoryMovements || []) {
    if (!inventoryMovements.some((row) => row.id === movement.id)) inventoryMovements.unshift(movement);
  }
  if (projection.cashDrawerSession) {
    cashDrawerSession = { ...projection.cashDrawerSession };
    delete cashDrawerSession.id;
  }
  syncAppGlobals?.();
}

async function restoreInventoryForCase(c) {
  ensureExtSettings();
  if (!settings.inventory.enabled) return;
  const cups = parseFloat(c.cups) || 0;
  for (const item of inventoryItems.filter(i => i.active && (i.perSession || 0) > 0)) {
    const qty = (item.perSession || 0) * cups;
    if (qty > 0) await adjustInventoryStock(item.id, qty, 'استرجاع — حذف جلسة', c.id);
  }
}

async function toggleInventorySystem(enabled) {
  ensureExtSettings();
  settings.inventory.enabled = !!enabled;
  if (enabled && !inventoryItems.length) await seedHijamaInventory();
  await persistData('settings', settings);
...
async function saveInvStock() {
  ...
  await adjustInventoryStock(item.id, delta, window._invStockMode === 'in' ? 'إضافة مخزون' : 'صرف يدوي', '');
...
async function saveInvSupplier() {
  if (!requirePermission('inventory.edit', 'تعديل المخزون')) return;
  ...
  await persistData('inventorySuppliers', inventorySuppliers);
...
async function deleteInvSupplier(id) {
  if (!requirePermission('inventory.edit', 'تعديل المخزون')) return;
...
async function initHijamaInventory() {
  if (!requirePermission('inventory.edit', 'تعديل المخزون')) return;
...
async function saveMessagingApiSettings() {
...
async function recordCashMovement(amount, reason, meta, skipPermCheck) {
  ...
  await persistData('cashDrawerSession', cashDrawerSession);
  logSystem('cash', reason, fmtMoney(Math.abs(amount)), meta);
  refreshCashFloatPage();
}

async function onCasePaymentRecorded(c) {
  ensureExtSettings();
...
async function onCaseCashPayment(c, cashAmount, changeGiven) {
  await onCasePaymentRecorded(c);
}
...
async function setCashOpeningFloat() {
...
async function cashEndOfDayWithdraw() {
  if (!requirePermission('cash.edit', 'إدارة عهدة الكاش')) return;
  ...
  await recordCashMovement(-withdraw, 'سحب نهاية اليوم — فائض العهدة', { type: 'eod' });
  notify('✅ تم سحب الفائض');
}

async function cashResetToday() {
  if (!requirePermission('cash.edit', 'إدارة عهدة الكاش')) return;
...
async function extRestoreData(data) {
  // Inventory is merged via RestoreStaging — do not overwrite here.
  if (data.systemLogs) { systemLogs = data.systemLogs; await persistData('systemLogs', systemLogs); }
  if (data.cashDrawerSession) { cashDrawerSession = data.cashDrawerSession; await persistData('cashDrawerSession', cashDrawerSession); }
  if (data.importHistory) {
    importHistory = data.importHistory;
    await persistData('importHistory', importHistory);
  }
  if (typeof extRestoreLeaveData === 'function') await extRestoreLeaveData(data);
  if (typeof extRestoreLedgerData === 'function') await extRestoreLedgerData(data);
  ensureExtSettings();
}
