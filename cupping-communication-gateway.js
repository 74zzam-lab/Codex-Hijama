...
let commBuiltinList = [];
let commCredentialStatus = { providers: {}, webhookSecret: false };

function getCommElectron() {
  return window.cuppingElectron || window.tadawiElectron || null;
}

async function commitCommunicationState(key, value) {
  const writer = window.SyncedWrite?.setTable;
  if (typeof writer !== 'function') throw new Error('authoritative_write_unavailable');
  const result = await writer.call(window.SyncedWrite, key, value);
  if (!result?.ok) throw new Error(result?.error || `communication_commit_failed:${key}`);
  return result;
}
...
async function migrateCommunicationSecretsToVault() {
  ensureCommunicationSettings();
...
async function initCommunicationGateway() {
  ...
  await migrateCommunicationSecretsToVault();
  await api.communication.init(getCommunicationConfigPayload());
  api.communication.onWebhook((event) => {
    handleCommWebhookEvent(event).catch((error) => {
      console.error('[communication] webhook persistence failed:', error);
    });
  });
  api.communication.onQueueUpdate(() => refreshCommQueueUI());
}

async function handleCommWebhookEvent(event) {
  const log = DB.get('communicationWebhookLog', []);
  log.unshift(event);
  if (log.length > 200) log.length = 200;
  await commitCommunicationState('communicationWebhookLog', log);
  if (event?.payload?.message_id || event?.payload?.status) {
    await updateMessageLogFromWebhook(event.payload);
  }
  refreshCommWebhookLogUI();
}

async function updateMessageLogFromWebhook(payload) {
  ...
  if (entry) {
    ...
    await commitCommunicationState('messageLog', messageLog);
...
async function saveCommunicationSettings() {
  ...
  if (webhookSecret) {
    ...
    if (!saved?.ok) {
      notify('❌ تعذر حفظ Webhook Secret في Windows Credential Vault', 'danger');
      return;
    }
...
async function saveCommProviderModal() {
  ...
  const apiKey = document.getElementById('comm-prov-key')?.value.trim() || '';
  const secret = document.getElementById('comm-prov-secret')?.value.trim() || '';
  if (apiKey || secret) {
    const credentials = {
      id: entry.id,
      ...(apiKey ? { apiKey } : {}),
    ...
    const saved = await getCommElectron()?.communication?.saveCredentials?.({ providers: [credentials] });
    if (!saved?.ok) {
      notify('❌ تعذر حفظ مفاتيح المزود في Windows Credential Vault', 'danger');
      return;
    }
    commCredentialStatus = saved;
  }
