...
context.currentUser = context.users[0];
...
CloudMeta.saveMeta({ ...CloudMeta.loadMeta(), centerId: cid, defaultBranchId: 'BR-MAIN', schemaVersion: 0 });

const repo = RepositoryFactory.createRepository(RepositoryFactory.createLocalStorageAdapter(context.DB));
context.Repository = repo;
context.SqliteBridge = {
  CORE_TABLES: ['clientsRegistry', 'cases', 'bookings', 'doctors', 'attendance', 'expenses'],
  OPERATIONAL_KEYS: new Set(repo.SYNCED_TABLES || []),
  KV_MIRROR: ['users', 'settings', 'packages', 'services'],
  async setAuthoritative(key, value) {
    repo.setAll(key, value);
    if (key === 'settings') context.settings = value;
    if (key === 'users') context.users = value;
    return { ok: true, table: key, authoritative: true };
  },
  async upsertRecord(key, value) {
    repo.upsert(key, value);
    return { ok: true, table: key, authoritative: true };
  },
  getSnapshot() {
    const store = context.DB.raw || context.DB;
    return { data: Object.fromEntries((repo.SYNCED_TABLES || []).map((key) => [key, store.get(key, [])])) };
  },
};
context.persistData = async (key, value) => context.SyncedWrite.setTable(key, value);
context.SqliteOutboxBridge = {
  async pushPending() { return { ok: true, flushed: 0, results: [] }; },
};
...
assert(CloudMeta.loadMeta().operationalMigrationDelegated === 'sqlite_003_p0b_authority', 'operational migration delegated to SQLite');
...
(async () => {
  // This suite exercises Cloud V2 data paths. License issuance is covered by
  // the offline-admin production suite and is unavailable in customer runtime.
  LicenseCloud.buildFromRecord = async (record, options) => ({
    schemaVersion: 6,
    keyId: 'cloud-v2-test-only',
    ...record,
    centerId: options?.centerId || record.centerId,
    branches: [{ id: 'BR-MAIN', name: 'Main', active: true }],
    signature: 'test-only-not-used-for-issuance',
  });
  LicenseCloud.verifyLicenseDoc = async () => ({ ok: true, testOnly: true });
  ...
  assert(!pushRes.ok && pushRes.error === 'legacy_full_table_writer_disabled',
    'legacy full-table push is disabled');

  const remotePath = OperationalLayer.drivePathForTable(cid, 'BR-MAIN', 'cases');
  driveStore[remotePath] = JSON.stringify(exported);
  assert(driveStore[remotePath], 'remote operation fixture available in mock drive');
  ...
  // A strict branch database pull must include every advertised table.
  // Missing files are an incomplete restore, not a successful empty pull.
  for (const table of OperationalLayer.OPERATIONAL_TABLES) {
    const tablePath = OperationalLayer.drivePathForTable(cid, 'BR-MAIN', table);
    if (!driveStore[tablePath]) {
      driveStore[tablePath] = JSON.stringify({
        centerId: cid, branchId: 'BR-MAIN', table, revision: 0, records: []
      });
    }
  }

  const pollRes = await SyncEngine.applyRemoteVersions(remoteVersions);
  assert(pollRes.ok, 'apply remote versions: ' + JSON.stringify(pollRes));
  ...
  assert(cross.pulled.some(p => p.branchId === 'BR-MAIN'), 'poll pulls locked branch only: ' + JSON.stringify(cross));
  ...
  assert(flushRes.ok, 'flush pending: ' + JSON.stringify(flushRes));

  CloudBootstrap.markBootstrapComplete('BR-MAIN');

  // Runtime initialization is tested independently from merge/conflict policy.
  // Keep this fixture on the explicit safe/offline analyzer branch so init can
  // prove its awaited sync/backup contract without inheriting earlier test data.
  context.DataStateAnalyzer.analyze = async () => ({ ok: true, blocked: false, offline: true });
  const init = await CloudV2.init({ force: true });
  ...
  assert(bootRes.ok, 'bootstrap hydrate new device: ' + JSON.stringify(bootRes));
  ...
  const v2AutoRes = await CloudV2.maybeAutoEnableCloudV2();
  assert(v2AutoRes.autoEnabled === true, 'maybeAutoEnableCloudV2 single branch multi-device: ' + JSON.stringify(v2AutoRes));
  ...
  assert(be1.ok && be1.branch?.id === 'BR-MAIN', 'first legacy V5 branch is atomically signed: ' + JSON.stringify(be1));
  ...
  assert(be2.ok && be2.branch?.id === 'BR02', 'second legacy V5 branch is atomically signed: ' + JSON.stringify(be2));
  const be3 = await context.BranchEnrollment.enrollBranch(LicenseCloud.loadLocal(), { branchName: 'مكة', source: 'owner_hub' });
  assert(!be3.ok && be3.error === 'branch_limit_reached', 'legacy V5 branch limit remains enforced: ' + JSON.stringify(be3));
