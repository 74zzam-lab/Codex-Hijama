...
async function main() {
  await scenario('O01-token-ttl-and-once', 'Setup token TTL + once-only redeem', async () => {
    ...
    return {
      ...
      reused: again.error,
      consumed: !!sb.OwnerBootstrap.loadState().tokenConsumedAt,
      signedLicenseImmutable: doc.ownerBootstrap.consumed === false,
    };
  });
  ...
  await scenario('O02-two-device-race', 'Two-device bootstrap race — one winner', async () => {
    ...
    const [r1, r2] = await Promise.all([
      ...
      a.OwnerBootstrap.redeemSetupToken(token, { username: 'b', password: 'Pass@123', recoveryCode: 'B' })
        .catch((e) => ({ ok: false, error: String(e && e.message || e) })),
    ]);
  ...
  await scenario('O03-emergency-recovery-audit', 'Emergency recovery authorized + audited', async () => {
    ...
    const salt = sb.OwnerBootstrap.loadState().recoverySalt;
  ...
  await scenario('L01-limits-grace-upgrade-downgrade', 'maxUsers + grace + upgrade/downgrade', async () => {
    ...
    if (down.ok || !down.requiresAdminReissue) throw new Error('unsigned_downgrade_allowed');
    ...
    if (up.ok || !up.requiresAdminReissue) throw new Error('unsigned_upgrade_allowed');
  ...
  await scenario('L02-license-persist-markers', 'License survive markers (uninstall-prep + lifecycle)', async () => {
    ...
    if (refresh.ok || !refresh.requiresAdminReissue) throw new Error('unsigned_refresh_allowed');
    return { uninstallPrepExit: unit.status, refreshRequiresAdminReissue: true };
  });
