...
const sandbox = {
  ...
  LicenseCloud: {
    ...
    async resignDoc(doc) { return { ...doc, signature: 'legacy-runtime-signature' }; },
  },
...
(async () => {
  ...
  check(created.ok === true && created.branch?.id === 'BR-MAIN',
    'owner_hub can atomically create a legacy V5 branch within the licensed limit');
