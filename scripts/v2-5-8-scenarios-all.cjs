...
async function main() {
  ...
  await scenario('A02-wizard-steps', 'Unified stepper Google→…→Ready', async () => {
    ...
    // V2-5.10 restores first, then creates Owner only when the restored data
    // did not already contain a usable owner credential.
    const v2510 = /NEW_STEPS\s*=\s*\[\s*'language',\s*'google',\s*'license',\s*'organization',\s*'branch',\s*'restore',\s*'owner',\s*'sync',\s*'ready'\s*\]/.test(boot);
    const v2511 = /NEW_STEPS\s*=\s*\[\s*'language',\s*'license',\s*'google',\s*'organization',\s*'branch',\s*'restore',\s*'owner',\s*'sync',\s*'ready'\s*\]/.test(boot);
    const v2512 = /NEW_STEPS\s*=\s*\[\s*'language',\s*'license',\s*'google',\s*'discovery',\s*'organization',\s*'branch',\s*'restore',\s*'owner',\s*'sync',\s*'ready'\s*\]/.test(boot);
    ...
    const v2517 = /NEW_STEPS\s*=\s*\[\s*'language',\s*'license',\s*'google',\s*'discovery',\s*'path_decision',\s*'organization',\s*'owner',\s*'branch',\s*'device',\s*'business_setup',\s*'publication',\s*'restore',\s*'sync',\s*'ready'\s*\]/.test(boot);
    if (!v258 && !v259 && !v2510 && !v2511 && !v2512 && !v2513 && !v2514 && !v2515 && !v2516 && !v2517) throw new Error('NEW_STEPS mismatch');
    return {
      steps: v259 ? 8 : 9,
      version: v2517 ? 'v2-5.16-stage13' : (v2516 ? 'v2-5.15-stage12' : (v2515 ? 'v2-5.14-stage11' : (v2514 ? 'v2-5.13-stage9' : (v2513 ? 'v2-5.12-stage8' : (v2512 ? 'v2-5.11-stage7' : (v2511 ? 'v2-5.11-stage6' : (v2510 ? 'v2-5.10' : (v259 ? 'v2-5.9' : 'v2-5.8'))))))))
    };
  });
  ...
  await scenario('A04-duplicate-prevention', 'OAuth/license/branch/owner in-flight locks', async () => {
    ...
    const enrollment = read('cloud/branch-enrollment.js');
    if (!enrollment.includes('requiresAdminReissue')) throw new Error('unsigned branch entitlement allowed');
    return { locks: 4, branchEntitlementIssuerOwned: true };
  });
