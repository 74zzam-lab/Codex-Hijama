...
// ── Test 1: monthly isolation with attendance data ──
{
  ...
  const futureSync = await EL.syncMonth(future.month, future.year, { carryOver: true });
...
// ── Test 2: empty month should not create salary accruals ──
{
  ...
  const EL = runLedgerTests(g);
  EL.init();
  const synced = await EL.syncMonth(emptyM, emptyY, { carryOver: false });
...
// ── Test 3: paid source must not remain in next month carryover balance ──
{
  ...
  await EL.recordPayment({ doctorId: 'd1', month: prev.month, year: prev.year, amount: payAmt, paymentMethod: 'cash' });
...
// ── Test 4: سند button must print receipt voucher, not account statement ──
{
  ...
  EL.init();
  await EL.syncMonth(prev.month, prev.year, { carryOver: false });
  const pay = await EL.recordPayment({ doctorId: 'd1', month: prev.month, year: prev.year, amount: 100, paymentMethod: 'cash' });
