...
async function main() {
  ...
  await scenario('S03-token-encryption-revoke', 'SafeStorage-only token encryption + disconnect revoke', async () => {
    const ts = readSrc('electron/cloud-providers/token-store.js');
    if (!/safeStorage/.test(ts) || !/secure_storage_unavailable/.test(ts)) throw new Error('safeStorage fail-closed path missing');
    if (/createHash\([^)]*sha256[^)]*\)[\s\S]{0,300}aes-256-gcm/.test(ts)) throw new Error('deterministic fallback encryption remains');
    ...
    return { safeStorageOnly: true, failClosed: true, revoke: true };
  });

  await scenario('S04-license-pull-public', 'License read public; mutation constrained and authenticated', async () => {
    const rbac = require('../electron/rbac-session.js');
    for (const ch of ['backup:listCloudBackups', 'backup:downloadCloudBackup', 'cache:readLicense', 'backup:uploadActivationArtifact']) {
      if (!rbac.sessionAllowsChannel(null, ch).ok) throw new Error('not public: ' + ch);
    }
    for (const ch of ['cache:writeLicense', 'backup:uploadCloud']) {
      if (rbac.sessionAllowsChannel(null, ch).ok) throw new Error('unexpected public mutation: ' + ch);
    }
    const gd = readSrc('electron/cloud-providers/google-drive.js');
    if (!/create:\s*false/.test(gd)) throw new Error('list/download must not create folders');
    return { licenseReadPublic: true, mutationGated: true, activationUploadConstrained: true, noCreateSideEffect: true };
  });
