...
async function main() {
  ...
  await scenario('R03-ipc-session-policy', 'IPC session required + rank policy', async () => {
    ...
    const proof = rbac.issueAuthenticationProof(ev, { userId: 'o1', role: 'owner', sessionEpoch: 0 });
    const bind = rbac.bindSession(ev, { userId: 'o1', role: 'owner', authProof: proof.proof, lookupUsers: () => users });
