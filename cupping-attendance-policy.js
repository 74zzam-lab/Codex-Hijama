...
(function (global) {
  ...
  async function saveAttPolicyFromUI() {
    ...
    const settingsCommit = await global.persistData?.('settings', global.settings);
    if (!settingsCommit || settingsCommit.ok === false) {
      global.notify?.('⚠️ تعذر حفظ سياسة الحضور', 'danger');
      return false;
    }
    if (typeof global.recalcAllAttendancePolicy === 'function' && global.attendance) {
      ...
      const attendanceCommit = await global.persistData?.('attendance', global.attendance);
      if (!attendanceCommit || attendanceCommit.ok === false) {
        global.notify?.('⚠️ تعذر حفظ تحديثات الحضور', 'danger');
        return false;
      }
    }
