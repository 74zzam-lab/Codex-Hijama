...
function readyEnv(overrides = {}) {
  ...
  const snap = {
    ...
    wizard: {
      path: 'new', currentStep: 0, restoreChoice: 'empty', syncDone: false, completedSteps: [], lang: 'ar', wizardFlowVersion: 13,
...
// 1. Current runtime ordering matches Stage 7 registry
(() => {
  const ctx = readyEnv();
  const ro = ctx.BootstrapGates.runtimeOrderingUnchanged();
  check(ro.unchanged === true, 'runtime NEW/EXISTING ordering matches registry');
...
(() => {
  const ctx = readyEnv({
    ...
    wizard: {
      path: 'new', lang: 'ar', restoreChoice: 'empty', currentStep: 4, completedSteps: [], wizardFlowVersion: 11,
      discoveryCompletedAt: new Date().toISOString(),
      licenseDiscoveryAttempted: true,
      cloudDiscovery: { result: { ok: true, status: 'no_existing_business' }, googleAccountKey: null },
    },
  ...
  check(first && first.id === 'OWNER_RESOLVED', 'partial NEW first unresolved = OWNER (Stage 9 before branch)');
  const runtimeSteps = ctx.BootstrapGates.getCurrentRuntimeSteps('new');
  check(runtimeSteps.indexOf('owner') < runtimeSteps.indexOf('branch'), 'runtime owner before branch (Stage 9)');
})();
...
(() => {
  const ctx = readyEnv({
    wizard: {
      path: 'existing', lang: 'ar', restoreChoice: 'cloud', currentStep: 0, wizardFlowVersion: 11,
      discoveryCompletedAt: new Date().toISOString(),
      licenseDiscoveryAttempted: true,
      cloudDiscovery: { result: { ok: true, status: 'no_existing_business' }, googleAccountKey: null },
    },
...
(() => {
  const ctx = readyEnv({
    ...
    deviceConfig: { deviceUuid: '', lockedBranchId: '', deviceName: '' },
    wizard: {
      path: 'new', currentStep: 7, lang: 'ar', restoreChoice: 'empty',
      completedSteps: ['language', 'google', 'discovery', 'license', 'organization'], wizardFlowVersion: 9,
      discoveryCompletedAt: new Date().toISOString(),
      licenseDiscoveryAttempted: true,
      cloudDiscovery: { result: { ok: true, status: 'no_existing_business' }, googleAccountKey: null },
    },
  ...
  const first = ctx.BootstrapGates.firstUnresolvedTargetGate('new');
  check(first && first.id === 'OWNER_RESOLVED', 'restart model finds owner gap before branch (Stage 9)');
  const resume = ctx.BootstrapCoordinator.resolveResumeStepIndex('new', 7);
  check(resume <= 6, 'coordinator resume not stale step 7');
})();

// 18. Stage 6/7 NEW runtime: license before google; EXISTING google before license (discovery between on Stage 7)
(() => {
...
(() => {
  const BG = require('../../cloud/bootstrap-gates');
  check(BG.getStepInventory().length >= 10, 'step inventory');
...
console.log('PASS stage-5-bootstrap-gate-map (gate model prepared, Stage 7 discovery runtime, zero-write predicates)');
process.exit(0);
