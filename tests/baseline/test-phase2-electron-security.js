...
async function main() {
  ...
  try {
    ...
    const runtimeCsp = windowPolicy.buildContentSecurityPolicy(root);
    check(!/script-src[^;]*'unsafe-inline'/.test(runtimeCsp), 'CSP blocks unsafe inline script execution');
    check(/script-src-attr 'unsafe-hashes'/.test(runtimeCsp), 'CSP permits only hashed legacy handlers');
  ...
  try {
    ...
    licThrow = e.code === 'license_mutation_requires_offline_admin_tool'
      || e.code === 'license_data_writable_root_not_configured'
      || e.code === 'PATH_TRAVERSAL' || /invalid|traversal/i.test(e.message);
  }
  check(licThrow, 'license shard mutation rejected before filesystem access');
