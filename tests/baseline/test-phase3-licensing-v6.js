...
async function main() {
  ...
  check(!fs.existsSync(path.join(root, 'tools/license-admin/keys/dev/ed25519-private.pem')), 'no private signing key is stored in source');
  ...
  const devPublicB64 = adminCrypto.publicKeySpkiB64(pub);

  const devGood = adminCrypto.issueLicense({
  ...
  check(adminCrypto.verifyLicense(devGood, pub) === true, 'admin verify valid development fixture');
  const fixtureDir = path.join(root, 'tools/license-admin/fixtures');
  const good = JSON.parse(fs.readFileSync(path.join(fixtureDir, 'TDW-PROD-TEST-000001.v6.json'), 'utf8'));
  const prodPublic = crypto.createPublicKey({
    key: Buffer.from('MCowBQYDK2VwAyEA+S+OvcvOUAdkH5Xcrh0s4wZ43kaGmyWEisCmRTy87WA=', 'base64'),
    type: 'spki',
    format: 'der',
  });
  check(adminCrypto.verifyLicense(good, prodPublic) === true, 'production fixture verifies with production public key');
  ...
  check(verified.ok === true, 'production client verifies production license');
  const rejectedDev = await CL.v6Verify.verifyPayload(devGood);
  check(rejectedDev.ok === false, 'production client rejects development-key license');
  ...
  const badExpired = await CL.v6Verify.verifyPayload(expired, {
    now: '2026-07-27T00:00:00.000Z',
    publicKeySpkiB64: devPublicB64,
  });
  check(badExpired.ok === false && badExpired.error === 'expired', 'expired license rejected');

  // Revoked
  CL.v6Verify.saveRevocations([{ licenseId: good.licenseId, reason: 'test' }]);
  ...
  const okBound = await CL.v6Verify.verifyPayload(bound, { fingerprint: fpA, publicKeySpkiB64: devPublicB64 });
  check(okBound.ok === true, 'matching device accepted');
  const badBound = await CL.v6Verify.verifyPayload(bound, { fingerprint: fpB, publicKeySpkiB64: devPublicB64 });
  ...
  const softOk = await CL.v6Verify.verifyPayload(soft, { fingerprint: fpClose, publicKeySpkiB64: devPublicB64 });
