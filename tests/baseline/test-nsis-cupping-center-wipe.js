...
const cleanWipe = nsh.match(/Function NT_WipeUserDataForCleanInstall([\s\S]*?)FunctionEnd/);
check(cleanWipe, 'clean-install wipe function required');
check(
  cleanWipe && !cleanWipe[1].includes('NT_StripLicenseFromArchive_Install'),
  'pre-install recovery archive must retain Local Storage/IndexedDB and other legacy data stores'
);
check(
  nsh.includes('complete recovery backup kept'),
  'clean-install confirmation must truthfully promise a complete recovery backup'
);
check(
  !nsh.includes('backup kept, license stripped'),
  'clean-install UI must not describe a destructively modified recovery backup'
);
