...
function main() {
  ...
  // Generic upload is authenticated; pre-login activation has a constrained channel.
  const uploadGate = rbac.sessionAllowsChannel(null, 'backup:uploadCloud');
  check(uploadGate.ok === false, 'generic uploadCloud gated pre-login');
  const activationUploadGate = rbac.sessionAllowsChannel(null, 'backup:uploadActivationArtifact');
  check(activationUploadGate.ok === true, 'constrained activation upload public pre-login');
  const cacheLicenseWriteGate = rbac.sessionAllowsChannel(null, 'cache:writeLicense');
  check(cacheLicenseWriteGate.ok === false, 'license cache write requires authenticated manager');
