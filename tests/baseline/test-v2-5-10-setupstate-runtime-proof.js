...
const configMergePromise = (async () => {
  const sandbox = {
    window: {},
    globalThis: {},
    users: [{
    ...
    DB: {
      ...
      set(k, v) { this._store[k] = v; return { ok: true, key: k }; },
    },
    async persistData(k, v) { this.DB.set(k, v); return { ok: true, key: k }; },
    SqliteBridge: {
      async setAuthoritative(k, v) {
        sandbox.DB.set(k, v);
        if (Object.prototype.hasOwnProperty.call(sandbox, k)) sandbox[k] = v;
        return { ok: true, key: k, authoritative: true };
      },
    },
    Repository: null,
    VersionsIndex: null,
  ...
  const res = await CL.importBranchPack(pack, { branchId: 'BR1', allowConflict: true });
  ...
  const res2 = await CL.importBranchPack(olderPack, { branchId: 'BR1', allowConflict: true });
...
configMergePromise.then(() => {
  ...
  console.error('FAIL v2-5.10 setupstate-runtime-proof', error);
  process.exit(1);
});
