...
function makeSnapshot(overrides = {}) {
  return {
    _meta: {
      ...
      centerId: 'CTR-SQLITE-TEST',
      branchId: 'BR-SQLITE-TEST',
    },
    clientsRegistry: [
...
async function main() {
  ...
  // Orphan visit is preserved in quarantine; it is never silently relinked.
  {
    const dbPath = tmpDb();
    const snap = makeSnapshot({
      cases: [
    ...
    const row = db.prepare(
      "SELECT payload_json, reason FROM p0b_quarantine WHERE source_table='cases' AND source_id='v9'"
    ).get();
    check(row?.reason === 'orphan_client_reference', 'orphan visit quarantined');
    check(JSON.parse(row?.payload_json || '{}').record?.total === 10, 'orphan visit payload preserved');
    db.close();
  }
  ...
  // Non-negative expense column (upsert clamps negatives)
  {
    ...
    const scope = { centerId: 'CTR-SQLITE-TEST', branchId: 'BR-SQLITE-TEST' };
    repos.expenses.upsert({ id: 'e0', amount: -5, date: '2026-07-01' }, scope);
  ...
  // Transaction rollback on hard failure: inject bad employee then attendance without employee by bypassing filter
  {
    ...
    try {
      const tx = db.transaction(() => {
        repos.clients.upsert({ id: 'c1', name: 'X' }, scope);
        // invalid attendance FK
        db.prepare(`INSERT INTO attendance
          (id, employee_id, date, payload_json, created_at, center_id, branch_id)
          VALUES ('a1','missing','2026-07-01','{}','2026-07-01',?,?)`
        ).run(scope.centerId, scope.branchId);
      });
      tx();
    } catch {
      rolled = true;
    ...
    check(repos.clients.count(scope) === 0, 'rollback left no clients');
    db.close();
  }
