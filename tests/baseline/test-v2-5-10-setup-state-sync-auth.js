...
check(/credentialRevision/.test(indexSrc)
  && /schedulePush\?\.\('users'[\s\S]{0,100}skipEnqueue:\s*true/.test(indexSrc)
  && /authoritative users\+revision\+outbox commit/.test(indexSrc),
  'password change enqueues users sync');
check(/ensureRbacSessionBound\(\)/.test(indexSrc)
  && /_pendingRbacProof/.test(indexSrc),
  'forced password persist uses proof-bound RBAC session');
check(/authenticateUser[\s\S]{0,500}_pendingRbacProof/.test(indexSrc),
  'forced password change re-authenticates and renews proof after session epoch change');
