...
check(
  src.includes("case 'owner': return ownerStepResolved();")
    || src.includes("case 'owner': return ownerSetupRequirementMet() && setupOwnerSessionReady();"),
  'owner step enforces credential (Stage 9 ownerStepResolved or legacy session gate)'
);
check(src.includes('function ownerStepResolved()'), 'ownerStepResolved present for NEW fresh path');
check(src.includes('setupOwnerSessionReady'), 'sync path still requires owner session binding');
