...
const licenseEngine = fs.readFileSync(path.join(root, 'license', 'engine', 'license-engine-v2.js'), 'utf8');
const errors = [];
...
check(!files.includes('license/**/*'), 'broad license glob must not package mutable/test data');
for (const safeLicensePath of [
  'license/api/**/*',
  'license/core/**/*',
  'license/engine/**/*',
  'license/migrations/**/*',
  ...
  'license/*.js',
  'license/*.json',
]) check(files.includes(safeLicensePath), `${safeLicensePath} must be packaged`);
check(!files.some((entry) => entry && typeof entry === 'object' && entry.from === 'license/data'),
  'license data FileSet must not be packaged');
check(!/fetchJson\(C\.DATA_BASE/.test(licenseEngine),
  'customer runtime must not fetch repository license fixture data');
