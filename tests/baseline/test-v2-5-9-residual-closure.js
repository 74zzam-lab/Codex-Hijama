...
sandbox.SqliteBridge = {
  async setAuthoritative(key, value) {
    store.set(key, value);
    if (Object.prototype.hasOwnProperty.call(sandbox, key)) sandbox[key] = value;
...
Promise.resolve(noMap).then(async (_r0) => {
...
async function runAsyncChecks() {
  ...
  bridgeSb.cuppingElectron = {
    database: {
      ...
      command: async () => { persistCalls += 1; return { ok: false, error: 'forced_fail' }; },
