...
check(hasCreate, 'repository global factory exposed');
...
function main2() {
  ...
  // Renderer-controlled source labels are not authorization capabilities.
  if (repo?.upsert) {
    const forged = repo.upsert('cases', { id: 'x2', branchId: 'BR-MAIN' }, { source: 'sync' });
    check(forged.ok === false && forged.error === 'not_authenticated', 'forged trusted sync source denied');
  }
  ...
  rbacSession.resetForTests();
  const issueProof = (event, user, extra = {}) => rbacSession.issueAuthenticationProof(event, {
  ...
  const bind = rbacSession.bindSession(fakeEvent, {
    ...
    authProof: issueProof(fakeEvent, sb.users[4]),
  ...
  check(tamperedBind.ok === false && tamperedBind.error === 'authentication_proof_required', 'IPC bind rejects forged role without proof');

  const ownerBind = rbacSession.bindSession(fakeEvent, {
    userId: '1', role: 'owner', branchScope: ['*'],
    authProof: issueProof(fakeEvent, sb.users[0]),
    lookupUsers: () => sb.users,
  });
  ...
  const emptyBind = rbacSession.bindSession(emptyKvEvent, {
    ...
    authProof: issueProof(emptyKvEvent, { id: 'owner-x', role: 'owner' }),
    lookupUsers: () => [],
  });
  check(emptyBind.ok === false && emptyBind.error === 'users_kv_empty', 'bind denies when users KV empty');
  const devEvent = { sender: { id: 100 } };
  const forgedDevBind = rbacSession.bindSession(devEvent, {
    userId: '__dev__', role: 'admin', branchScope: ['*'],
    lookupUsers: () => sb.users,
  });
  check(forgedDevBind.ok === false && forgedDevBind.error === 'authentication_proof_required', 'dev synthetic account requires trusted authentication');
  const devBind = rbacSession.bindSession(devEvent, {
    userId: '__dev__', role: 'admin', branchScope: ['*'],
    authProof: issueProof(devEvent, { id: '__dev__', role: 'admin' }, { isDev: true }),
    lookupUsers: () => sb.users,
  });
  check(devBind.ok === true, 'authenticated dev support session binds without KV row');
  const missingEvent = { sender: { id: 101 } };
  const skipIgnored = rbacSession.bindSession(missingEvent, {
    userId: 'missing', role: 'admin', branchScope: ['*'], skipLookup: true,
    authProof: issueProof(missingEvent, { id: 'missing', role: 'admin' }),
    lookupUsers: () => sb.users,
  });
