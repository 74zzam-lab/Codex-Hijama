...
const gdriveApi = fs.readFileSync(path.join(root, 'electron/cloud-providers/google-drive-api.js'), 'utf8');
...
check(/DISCOVERY_OVERALL_MS\s*=\s*180000/.test(mainDiscovery), '3-minute default discovery budget');
...
check(/onDownloadProgress/.test(preload), 'preload download progress listener');
check(/backup:downloadProgress/.test(preload), 'download progress channel allowlisted');
check(/backup:discoverCloudRestorePoints/.test(mainJs), 'main IPC handler');
check(/branchName/.test(mainJs) && /180000/.test(mainJs), 'main IPC branchName + 3-minute timeout cap');
...
check(/downloadFileWithProgress/.test(gdriveApi), 'google drive api supports streaming download progress');
check(/destPartialPath/.test(gdrive), 'google-drive supports streaming to partial file');
check(/onDownloadProgress|attachDownloadProgress/.test(rendererDiscovery), 'renderer wires cloud download progress');
check(/lastStageRatio/.test(rendererDiscovery), 'restore watchdog preserves last stage ratio');
...
assert.strictEqual(discovery.DISCOVERY_OVERALL_MS, 180000);
assert.strictEqual(discovery.clampTimeoutMs(240000), 180000);
