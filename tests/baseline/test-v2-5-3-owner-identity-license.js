...
function makeSandbox(sharedDoc) {
  ...
  const sandbox = {
    ...
    DB: {
      ...
      set(k, v) { mem[k] = v; return { ok: true, key: k }; },
    },
    async persistData(k, v) { mem[k] = v; return { ok: true, key: k }; },
    LicenseCloud: {
      _doc: sharedDoc,
...
async function main() {
  ...
  // ── OWN: token TTL / invalid / reuse / race / google ──
  {
    ...
    const bootstrapState = sb.OwnerBootstrap.loadState();
    check(!!bootstrapState.tokenConsumedAt, 'bootstrap consumed in operational state');
    check(!!bootstrapState.emergencyRecoveryHash, 'emergency hash stored in operational state');
    check(!!bootstrapState.recoverySalt, 'recovery salt stored in operational state');
    check(doc.ownerBootstrap.consumed === false, 'signed license bootstrap block remains immutable');
  ...
  // Owner recovery + emergency + unauthorized
  {
    ...
    const bootstrapState = sb.OwnerBootstrap.loadState();
    const emergencyHash = bootstrapState.emergencyRecoveryHash;
    const recoverySalt = bootstrapState.recoverySalt;
  ...
  // License limits: maxUsers, offline grace, upgrade/downgrade preserve data
  {
    ...
    check(down.ok === false && down.requiresAdminReissue === true && down.dataPreserved === true,
      'downgrade requires issuer reissue and preserves data');
    ...
    check(up.ok === false && up.requiresAdminReissue === true && up.dataPreserved === true,
      'upgrade requires issuer reissue and preserves data');
