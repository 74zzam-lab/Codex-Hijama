...
const sandbox = {
  ...
  DB: {
    ...
    set(k, v) {
      ...
      if (k === '__tdw_meta__') store.meta = { ...v };
...
sandbox.LicenseCloud.saveLocalCommitted = async (doc) => {
  store.cloud = doc;
  if (doc?.centerId) store.meta.centerId = doc.centerId;
  return { ok: true, doc };
};
...
(async () => {
  ...
  sandbox.BackupBridge.downloadCloudBackup = async (_p) => ({ ok: true, text: JSON.stringify(goodA) });
  res = await sandbox.CloudBootstrap.discoverAndFetchLicenseFromDrive({ forceList: true });
