...
check(!allowedTrusted.ok && allowedTrusted.error === 'branch_access_denied', 'renderer source labels must not bypass branch authorization');
...
(async () => {
  ...
  context.currentUser = reception;
  const deniedResolveAsReception = await ConflictQueue.resolve(pendingJed.id, { choice: 'local' });
  ...
  const deniedBranchResolve = await ConflictQueue.resolve(pendingJed.id, { choice: 'local' });
