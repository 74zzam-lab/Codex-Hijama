...
scenario('user_permission_change', async () => {
  context.currentUser = { id: 'u2', role: 'reception' };
  const r = await context.ConflictQueue.resolve('fake', { choice: 'local' });
...
scenario('synced_write_restore_path', async () => {
  ...
  const originalSetTable = context.SyncedWrite.setTable;
  context.SyncedWrite.setTable = async (table, value) => {
    const rows = Array.isArray(value) ? value : (value == null ? [] : [value]);
    context.DB.set(table, rows);
    if (table === 'cases') context.cases = rows;
    if (table === 'clientsRegistry') context.clientsRegistry = rows;
    return { ok: true, via: 'e2e_authoritative_stub', table };
  };
  try {
    ...
    assert((context.DB.get('cases', []) || []).some((c) => c.id === 'b1'), 'backup case applied through restore staging');
  } finally {
    context.SyncedWrite.setTable = originalSetTable;
  }
});
