 ...
 * NEW: Language → Activation → Google → Discovery → Path decision (if needed) → Organization → ...
...
(function (global) {
  ...
  const NEW_STEPS = ['language', 'license', 'google', 'discovery', 'path_decision', 'organization', 'branch', 'restore', 'owner', 'sync', 'ready'];
      ...
      const legacySteps = w.path === PATHS.EXISTING
        ...
        : (version < 6
          ...
          : (version < 7
            ...
            : (version < 8 ? LEGACY_NEW_STEPS_PRE_STAGE8 : NEW_STEPS)));
