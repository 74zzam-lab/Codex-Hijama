...
(() => {
  ...
  check(w.wizardFlowVersion === 8 || w.wizardFlowVersion === 7 || w.wizardFlowVersion === 6, 'legacy wizard migrated to flow v6+');
...
(() => {
  ...
  const expectedTail = steps.includes('path_decision')
    ? ['path_decision', 'organization', 'branch', 'restore', 'owner', 'sync', 'ready']
    : ['organization', 'branch', 'restore', 'owner', 'sync', 'ready'];
  check(JSON.stringify(tail) === JSON.stringify(expectedTail), 'NEW tail after discovery includes path_decision when Stage 8 fork gate present');
})();
