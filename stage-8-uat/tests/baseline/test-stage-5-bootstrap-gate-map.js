...
function readyEnv(overrides = {}) {
  ...
  const snap = {
    ...
    wizard: {
      path: 'new', currentStep: 0, restoreChoice: 'empty', syncDone: false, completedSteps: [], lang: 'ar', wizardFlowVersion: 8,
...
(() => {
  const ctx = readyEnv({
    ...
    wizard: {
      path: 'new', lang: 'ar', restoreChoice: 'empty', currentStep: 4, completedSteps: [], wizardFlowVersion: 8,
      discoveryCompletedAt: new Date().toISOString(),
      licenseDiscoveryAttempted: true,
      cloudDiscovery: { result: { ok: true, status: 'no_existing_business' }, googleAccountKey: null },
    },
...
(() => {
  const ctx = readyEnv({
    wizard: {
      path: 'existing', lang: 'ar', restoreChoice: 'cloud', currentStep: 0, wizardFlowVersion: 8,
      discoveryCompletedAt: new Date().toISOString(),
      licenseDiscoveryAttempted: true,
      cloudDiscovery: { result: { ok: true, status: 'no_existing_business' }, googleAccountKey: null },
    },
...
(() => {
  const ctx = readyEnv({
    ...
    wizard: {
      ...
      completedSteps: ['language', 'google', 'discovery', 'license', 'organization'], wizardFlowVersion: 8,
      discoveryCompletedAt: new Date().toISOString(),
      licenseDiscoveryAttempted: true,
      cloudDiscovery: { result: { ok: true, status: 'no_existing_business' }, googleAccountKey: null },
    },
