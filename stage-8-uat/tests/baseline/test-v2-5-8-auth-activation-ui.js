...
async function main() {
  ...
  check(AE.classifyTechnical({ error: 'setup_activation_commit_failed' }) === 'setup_activation_failed', 'setup activation error is not hidden as unknown');
  ...
  check(/NEW_STEPS\s*=\s*\[[^\]]*language[^\]]*license[^\]]*google[^\]]*organization[^\]]*branch/.test(bootCode.replace(/\s+/g, ' ')), 'new steps order core (Stage 6 activation-first)');
  ...
  check(/schemaVersion/.test(be) && /requiresAdminReissue/.test(be)
      && /license_document_immutable_admin_signature_required/.test(be)
      && /commitLicenseRevision/.test(be),
    'legacy V5 branch creation is atomic while V6 remains issuer-owned');
