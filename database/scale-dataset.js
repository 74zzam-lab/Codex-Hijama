...
function generateScaleDataset(db, options) {
  ...
  const centerId = String(options?.centerId || 'CTR-SCALE');
  const branchId = String(options?.branchId || 'BR-SCALE');
  ...
  const insClient = db.prepare(
    `INSERT OR IGNORE INTO clients
      (id, file_no, name, phone, center_id, branch_id, payload_json, created_at, updated_at)
     VALUES (@id, @file_no, @name, @phone, @center_id, @branch_id, @payload_json, @created_at, @updated_at)`
  );
  bulkInsert(db, (i) => {
    const n = i + 1;
    const id = `c${pad(n, cw)}`;
    insClient.run({
      ...
      center_id: centerId,
      branch_id: branchId,
      payload_json: JSON.stringify({ id, centerId, branchId, name: `Client ${n}` }),
      created_at: now,
      updated_at: now,
    });
  }, targets.clients);
  ...
  const insVisit = db.prepare(
    `INSERT OR IGNORE INTO visits
      (id, invoice, client_id, date, total, center_id, branch_id, payload_json, created_at, updated_at)
     VALUES (@id, @invoice, @client_id, @date, @total, @center_id, @branch_id, @payload_json, @created_at, @updated_at)`
  );
  bulkInsert(db, (i) => {
    const n = i + 1;
    ...
    const id = `v${pad(n, vw)}`;
    const clientId = `c${pad(clientN, cw)}`;
    insVisit.run({
      id,
      ...
      center_id: centerId,
      branch_id: branchId,
      payload_json: JSON.stringify({ id, centerId, branchId, clientRegistryId: clientId, date: `2024-${month}-${day}` }),
      created_at: now,
      updated_at: now,
    });
  }, targets.visits);
  ...
  const insInvoice = db.prepare(
    `INSERT OR IGNORE INTO invoices
      (id, visit_id, invoice_number, total, center_id, branch_id, payload_json, created_at)
     VALUES (@id, @visit_id, @invoice_number, @total, @center_id, @branch_id, @payload_json, @created_at)`
  );
  const invoiceN = Math.min(targets.invoices, targets.visits);
  bulkInsert(db, (i) => {
    const n = i + 1;
    const id = `inv${pad(n, vw)}`;
    const visitId = `v${pad(n, vw)}`;
    insInvoice.run({
      id,
      visit_id: visitId,
      ...
      center_id: centerId,
      branch_id: branchId,
      payload_json: JSON.stringify({ id, centerId, branchId, visitId, invoiceNumber: `NUM-${pad(n, vw)}` }),
      created_at: now,
    });
  }, invoiceN);
  ...
  const insAppt = db.prepare(
    `INSERT OR IGNORE INTO appointments
      (id, client_id, date, time, status, center_id, branch_id, payload_json, created_at, updated_at)
     VALUES (@id, @client_id, @date, @time, @status, @center_id, @branch_id, @payload_json, @created_at, @updated_at)`
  );
  bulkInsert(db, (i) => {
    const n = i + 1;
    ...
    insAppt.run({
      ...
      center_id: centerId,
      branch_id: branchId,
      payload_json: JSON.stringify({ id, centerId, branchId, clientId, date: `2024-06-${day}` }),
      created_at: now,
      updated_at: now,
    });
  ...
  const insAtt = db.prepare(
    `INSERT OR IGNORE INTO attachments
      (id, entity_type, entity_id, path, mime, center_id, branch_id, payload_json, created_at)
     VALUES (@id, @entity_type, @entity_id, @path, @mime, @center_id, @branch_id, @payload_json, @created_at)`
  );
  bulkInsert(db, (i) => {
    const n = i + 1;
    const clientN = (i % Math.max(1, targets.clients)) + 1;
    const id = `att${pad(n, vw)}`;
    const entityId = `c${pad(clientN, cw)}`;
    insAtt.run({
      id,
      entity_type: 'client',
      entity_id: entityId,
      ...
      center_id: centerId,
      branch_id: branchId,
      payload_json: JSON.stringify({ id, centerId, branchId, entityType: 'client', entityId }),
      created_at: now,
    });
  }, targets.attachments);
