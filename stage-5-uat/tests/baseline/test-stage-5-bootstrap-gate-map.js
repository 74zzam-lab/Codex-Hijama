...
function readyEnv(overrides = {}) {
  ...
  const snap = {
    ...
    wizard: { path: 'new', currentStep: 0, restoreChoice: 'empty', syncDone: false, completedSteps: [], lang: 'ar' },
...
(() => {
  const ctx = readyEnv({
    wizard: { path: 'new', currentStep: 8, completedSteps: ['language', 'google', 'license', 'organization', 'branch', 'restore', 'owner', 'sync', 'ready'], syncDone: true, restoreChoice: 'empty', lang: 'ar' },
    deviceConfig: { deviceUuid: '', lockedBranchId: '', deviceName: '' },
    meta: {},
    users: [],
  });
...
(() => {
  const ctx = readyEnv({
    ...
    deviceConfig: { deviceUuid: '', lockedBranchId: '', deviceName: '' },
    wizard: { path: 'new', currentStep: 7, lang: 'ar', restoreChoice: 'empty', completedSteps: ['language', 'google', 'license', 'organization'] },
  });
  const first = ctx.BootstrapGates.firstUnresolvedTargetGate('new');
...
// 18. Runtime path not reordered (manifest)
(() => {
  const bootSrc = fs.readFileSync(path.join(root, 'cloud/boot-flow-ui.js'), 'utf8');
  check(/const NEW_STEPS = \['language', 'google', 'license'/.test(bootSrc), 'NEW runtime google before license unchanged');
  check(/'branch', 'restore', 'owner', 'sync'/.test(bootSrc), 'branch before owner unchanged');
})();
