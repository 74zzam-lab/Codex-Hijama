...
(function (global) {
  ...
  function evaluateDiscoveryResolved(path) {
    const activation = evaluateActivationResolved();
    if (activation.status === GATE_STATUS.RESOLVED) {
      return gateResult('DISCOVERY_RESOLVED', GATE_STATUS.RESOLVED, 'license/activation present (discovery or manual)', activation.source);
    }
    const w = wizardRaw() || {};
    if (w.licenseDiscoveryAttempted === true || w.cloudDiscovery) {
      return gateResult('DISCOVERY_RESOLVED', GATE_STATUS.RESOLVED, 'explicit discovery attempt recorded', '__tdw_boot_wizard__');
    }
    if (path === PATH_EXISTING && w.path === PATH_EXISTING && BF()?.hasGoogle?.()) {
      return gateResult('DISCOVERY_RESOLVED', GATE_STATUS.MISSING, 'existing path chosen but license not recovered', 'wizard.path+license');
    }
    if (BF()?.hasGoogle?.()) {
      return gateResult('DISCOVERY_RESOLVED', GATE_STATUS.MISSING, 'google connected — awaiting license discovery or manual key', 'embedded google discovery');
    }
  ...
  function getCurrentRuntimeSteps(path) {
    const BFm = BF();
    if (BFm?.getStepManifest) {
      const m = BFm.getStepManifest();
  ...
  function runtimeOrderingUnchanged() {
    const BFm = BF();
    const manifest = BFm?.getStepManifest?.() || {};
  ...
  function getStepInventory() {
    return [
      ...
      { stepId: 'google', displayName: 'ربط Google', path: 'BOTH', currentPositionNew: 1, currentPositionExisting: 1, uiRenderer: 'renderStepUI:google', entry: 'runGoogleConnect', completionChecker: 'hasGoogle()', sourceOfTruth: 'DriveAdapter+settings.backup.providers.google', writes: ['settings oauth', 'wizard path/completedSteps on discovery'], cloudEffect: true, sqliteEffect: true, canRetry: true, canResume: true, canSkip: false, dependsOn: ['language'], produces: ['google session', 'embedded license discovery'] },
      { stepId: 'license', displayName: 'التفعيل والترخيص', path: 'BOTH', currentPositionNew: 2, currentPositionExisting: 2, uiRenderer: 'renderStepUI:license', entry: 'activateLicenseKey / autoDiscover', completionChecker: 'hasValidLicense()', sourceOfTruth: '__tdw_cloud_license__+LicenseActivationGate', writes: ['setupCommitSignedActivation', 'activation state'], cloudEffect: true, sqliteEffect: true, canRetry: true, canResume: true, canSkip: false, dependsOn: ['google'], produces: ['consumed activation'] },
      { stepId: 'organization', displayName: 'المؤسسة', path: 'BOTH', currentPositionNew: 3, currentPositionExisting: 3, uiRenderer: 'renderStepUI:organization', entry: 'commitSetupOrganizationDevice({centerName})', completionChecker: 'hasCenterData()', sourceOfTruth: 'centerId+centerName license/settings', writes: ['meta setupOrganizationDeviceCommittedAt', 'settings'], cloudEffect: false, sqliteEffect: true, canRetry: true, canResume: true, canSkip: false, dependsOn: ['license'], produces: ['organization metadata'] },
      { stepId: 'branch', displayName: 'إنشاء أول فرع', path: 'NEW', currentPositionNew: 4, uiRenderer: 'renderStepUI:branch', entry: 'createFirstBranchFromForm / bindExistingBranch', completionChecker: 'hasBranch()&&hasDeviceBranch()', sourceOfTruth: 'license.branches+DeviceConfig', writes: ['device config', 'device registry', 'license branches', 'RESTART_REQUIRED'], cloudEffect: false, sqliteEffect: true, canRetry: true, canResume: true, canSkip: false, dependsOn: ['organization'], produces: ['branch', 'device registration'] },
      { stepId: 'branch_select', displayName: 'اختيار فرع موجود', path: 'EXISTING', currentPositionExisting: 4, uiRenderer: 'renderStepUI:branch_select', entry: 'bindExistingBranch', completionChecker: 'hasBranch()&&hasDeviceBranch()', sourceOfTruth: 'license.branches+DeviceConfig', writes: ['device bind only'], cloudEffect: false, sqliteEffect: true, canRetry: true, canResume: true, canSkip: false, dependsOn: ['organization'], produces: ['device registration'] },
      { stepId: 'restore', displayName: 'مصدر البيانات', path: 'BOTH', currentPositionNew: 5, currentPositionExisting: 5, uiRenderer: 'renderStepUI:restore', entry: 'runDiscovery / restore actions', completionChecker: 'hasRestoreDecision()', sourceOfTruth: 'wizard.restoreChoice+RestoreReconciliation', writes: ['restoreChoice', 'optional full DB replace'], cloudEffect: true, sqliteEffect: true, canRetry: true, canResume: true, canSkip: false, dependsOn: ['branch/device', 'google for cloud'], produces: ['data source decision'] },
      { stepId: 'owner', displayName: 'حساب المالك', path: 'BOTH', currentPositionNew: 6, currentPositionExisting: 6, uiRenderer: 'renderStepUI:owner', entry: 'createOwnerFromWizard / authenticateExistingOwnerFromWizard', completionChecker: 'ownerSetupRequirementMet&&setupOwnerSessionReady', sourceOfTruth: 'OwnerManagement+users+RBAC session', writes: ['users owner profile'], cloudEffect: false, sqliteEffect: true, canRetry: true, canResume: true, canSkip: false, dependsOn: ['restore decision', 'branch/device'], produces: ['owner credential', 'setup session'] },
      { stepId: 'sync', displayName: 'المزامنة الأولية', path: 'BOTH', currentPositionNew: 7, currentPositionExisting: 7, uiRenderer: 'renderStepUI:sync', entry: 'runInitialSyncPipeline', completionChecker: 'hasSyncDone()', sourceOfTruth: 'meta.bootstrapCompletedAt', writes: ['bootstrapCompletedAt', 'SyncEngine'], cloudEffect: true, sqliteEffect: true, canRetry: true, canResume: true, canSkip: false, dependsOn: ['owner session', 'restoreChoice', 'google'], produces: ['initial sync complete'] },
      { stepId: 'ready', displayName: 'الجاهزية', path: 'BOTH', currentPositionNew: 8, currentPositionExisting: 8, uiRenderer: 'renderStepUI:ready', entry: 'finalize + relaunch', completionChecker: 'isBootComplete/evaluateReady', sourceOfTruth: 'ReadyPureEvaluator', writes: ['setupCompletedAt', 'BOOT_DONE_KEY'], cloudEffect: false, sqliteEffect: true, canRetry: true, canResume: true, canSkip: false, dependsOn: ['all gates'], produces: ['READY durable'] },
    ];
  }
  ...
  function getCapabilityMatrix() {
    ...
    return caps.map((cap) => {
      ...
      if (cap === 'activation') {
        row.currentNew = 2; row.targetNew = 0; row.currentExisting = 2; row.targetExisting = 'recovery'; row.changeRequired = 'Stage 6: move before Google (NEW only)';
      } else if (cap === 'google') {
        row.currentNew = 1; row.targetNew = 1; row.currentExisting = 1; row.targetExisting = 0; row.changeRequired = 'Stage 6 reorder (NEW)';
      } else if (cap === 'discovery') {
        row.currentNew = 'embedded@google'; row.targetNew = 2; row.currentExisting = 'embedded@google'; row.targetExisting = 1; row.changeRequired = 'Stage 7 explicit step';
      } else if (cap === 'owner') {
        ...
        row.currentNew = 4; row.targetNew = 5; row.currentExisting = 4; row.targetExisting = 'select'; row.changeRequired = 'Stage 9 after owner';
      } else if (cap === 'device') {
        row.currentNew = 'inside branch@4'; row.targetNew = 6; row.currentExisting = 'inside branch_select@4'; row.targetExisting = 4; row.changeRequired = 'Stage 11 explicit step';
      } else if (cap === 'login') {
  ...
  function getDependencyGraph() {
    return {
      hard: [
        { from: 'language', to: 'google', reason: 'wizard navigation' },
        { from: 'google', to: 'license', reason: 'initialSetupRequiresGoogle + Drive discovery' },
        { from: 'license', to: 'organization', reason: 'centerId from license' },
      ...
      accidental: [
        ...
        { from: 'google', to: 'activation', reason: 'current NEW starts Google before license step' },
  ...
  function getStage6Plan() {
    return {
      goal: 'NEW: Activation before Google (runtime reorder)',
      files: [
        'cloud/boot-flow-ui.js',
        'cloud/bootstrap-coordinator.js',
        'cloud/bootstrap-gates.js',
        'index.html (script order only if needed)',
        'tests/baseline/test-stage-6-activation-before-google.js',
      ],
      functions: [
        'NEW_STEPS array reorder: license before google',
        'validateStep forward navigation guards',
        'autoDiscoverActivationAfterGoogle trigger point (post-google only today)',
        'SETUP_CONNECTIVITY_POLICY review',
      ],
      transitionChanges: [
        'language → license (activation) → google → ...',
        'Defer Drive license discovery until after Google still required for pull',
      ],
      compatibilityRisks: [
        'wizard KV currentStep indices shift',
        'resume index migration for in-progress setups',
        'google discovery assumes license step already passed in some callbacks',
      ],
      tests: [
        'NEW runtime order assertion',
        'EXISTING path unchanged',
        'Stage 1-5 regressions',
        'google-state-recovery',
      ],
      rollback: 'Revert NEW_STEPS + resume migration shim; coordinator gates unchanged',
      feasibility: 'surgical with resume migration — EXISTING path untouched in Stage 6',
    };
  }
