(function (global) {
  ...
  async function applyActivation(code, errEl, okEl) {
    ...
    if (typeof global.LicenseActivationGate !== 'undefined' && validation.record) {
      try {
        ...
        if (!committed || committed.ok !== true) {
          const error = committed?.error || 'activation_commit_failed';
          if (errEl) {
            errEl.textContent = '✗ تعذّر إكمال حفظ الترخيص: ' + error;
            errEl.style.display = 'block';
          }
          return { handled: true, ok: false, error, activationCommit: committed || null };
        }
        ...
        if (errEl) {
          errEl.textContent = '✗ تعذّر إكمال حفظ الترخيص: ' + (e?.message || String(e));
          errEl.style.display = 'block';
        }
        return { handled: true, ok: false, error: e?.message || 'activation_commit_failed' };
      }
    }
    ...
    return { handled: true, ok: true, lic, payload, validation, activationCommit };
  }
