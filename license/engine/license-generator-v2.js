(function (global) {
  ...
  async function generate(config) {
    ...
    const record = {
      ...
      branchesList: Array.isArray(config.branchesList) ? config.branchesList : null,
    ...
    let driveDocument = null;
    if (global.LicenseCloud?.buildFromRecord) {
      try {
        const featureKeys = (resolved.featureIds || []).map((id) => {
          const feature = (CL.registries?.feature?.features || []).find((item) => item.id === id);
          return feature?.key || id;
        });
        driveDocument = await global.LicenseCloud.buildFromRecord(record, {
          centerName: record.customer.company || record.customer.name,
          features: featureKeys,
          persistCenterId: false
        });
      } catch { /* Drive document remains optional until activation. */ }
    }
