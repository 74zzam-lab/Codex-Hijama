...
(function (global) {
  ...
  async function verifyPayload(raw, opts) {
    ...
    if (!options.publicKeySpkiB64 && lic.keyId !== CL.v6Constants?.KEY_ID) {
      return { ok: false, error: 'key_id', reasonCode: 'license_key_id_invalid', userMessage: 'تعذّر التحقق من جهة إصدار الترخيص' };
    }
  ...
  function toLegacyRuntimeLicense(verified, codeOrRaw) {
    ...
    const enabledKeys = new Set((lic.features || []).map((item) => String(item)));
    const runtimeFeatures = {};
    (global.FEATURE_REGISTRY || []).forEach((feature) => {
      runtimeFeatures[feature.id] = feature.tier === 'core'
        || enabledKeys.has(String(feature.key))
        || enabledKeys.has(String(feature.id));
    });
    return {
      type: 'renew',
      licType: 'renew',
