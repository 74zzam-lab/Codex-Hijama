(function (global) {
  ...
  async function validateKey(key, bundleOverride) {
    ...
    const bundlePackageId = String(bundle.packageId || '').padStart(2, '0');
    const bundleSubscription = parseInt(String(bundle.subscriptionId || '').replace(/\D/g, ''), 10) || 0;
    const bundleAction = parseInt(String(bundle.actionId || '').replace(/\D/g, ''), 10) || 0;
    if (bundlePackageId !== decoded.packageId
        || bundleSubscription !== decoded.fields.subscriptionId
        || Math.min(bundleAction, 7) !== decoded.fields.actionId) {
      return { ok: false, error: 'key_bundle_mismatch', decoded };
    }

    if (!record) record = CL.store.getLicense(bundle.licenseId);
    // The authenticated bundle is authoritative. V5's compact date field is
    // only 13 bits and historical lifetime/custom dates may have wrapped.
    const expiry = bundle.expiryDate || record?.expiryDate || decoded.expiry;
