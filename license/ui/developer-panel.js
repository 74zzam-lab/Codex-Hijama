...
(function (global) {
  ...
  async function renderDowngradePanel() {
    ...
    host.innerHTML = `
      ...
      <div class="lic-v2-actions-row" style="margin-top:10px">
        <button type="button" class="lic-v2-btn lic-v2-btn-secondary" onclick="CommercialLicense.drawer?.copyVaultActivationRow?.(window._lastDowngradeLicenseResult)">ðŸ“Š صف activations</button>
        <button type="button" class="lic-v2-btn lic-v2-btn-secondary" onclick="CommercialLicense.drawer?.copyVaultBundleRow?.(window._lastDowngradeLicenseResult)">ðŸ“¦ bundle للشيت</button>
      </div>
  ...
  async function runDowngrade() {
    ...
    try {
      ...
      global._lastDowngradeLicenseResult = result;
      if (out) {
