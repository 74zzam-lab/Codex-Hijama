...
<div id="lic-v2-overlay" class="lic-v2-overlay" aria-hidden="true">
  ...
  function open(opts) {
    ...
    if (_generationInFlight) {
      licV2Toast('انتظر اكتمال توليد الترخيص الحالي', 'warning');
      return false;
    }
  ...
  function renderBatchResult(body, batchResult) {
    ...
    body.innerHTML = `
      ...
      <p class="lic-v2-step-footnote">الصق activations من <strong>A2</strong> · bundles من <strong>A2</strong> (اترك صف العناوين) — كل سطر = صف، Tab يفصل الأعمدة · سلّم أي كود <code>unused</code> للعميل.</p>`;
    el('lic-v2-copy-all-keys').onclick = () => copyText(keys.join('\n'), 'تم نسخ ' + batchResult.count + ' مفتاح');
    el('lic-v2-copy-vault-rows').onclick = () => copyText(formatBatchVaultActivationRows(batchResult), 'تم نسخ ' + batchResult.count + ' صف — الصق في تبويب activations من A2');
    el('lic-v2-copy-vault-bundles').onclick = () => copyText(formatBatchVaultBundleRows(batchResult), 'تم نسخ ' + batchResult.count + ' صف — الصق في تبويب bundles من A2');
  ...
  function renderStep5(body) {
    ...
    if (_result) {
      ...
      el('lic-v2-copy-vault-row').onclick = () => copyText(formatVaultActivationRow(_result), 'تم نسخ صف activations — الصق في Google Sheet من A2');
      el('lic-v2-copy-vault-bundle').onclick = () => copyText(formatVaultBundleRow(_result), 'تم نسخ bundle — الصق في تبويب bundles من A2');
  ...
  function formatSummaryText(result) {
    const s = result.summary;
    const schemaVersion = Number(result?.record?.licenseSchemaVersion)
      || (/^TDW6\./.test(String(result?.key || '')) ? 6 : 5);
    return [
      `ترخيص Hijama Management System V${schemaVersion}`,
      `المفتاح: ${result.key}`,
      `المعرّف: ${s.licenseId}`,
  ...
  async function generateLicense() {
    if (_generationInFlight) return { ok: false, error: 'generation_in_flight' };
    collectStep4();
    ...
    try {
      if (count > 1 && !CL.generator?.generateBatch) {
        ...
        return { ok: false, error: 'batch_generation_unavailable' };
      }
      if (count > 1 && CL.generator.generateBatch) {
      ...
      return { ok: false, error: e?.message || 'generation_failed' };
    } finally {
  ...
  function render() {
    ...
    const closeButton = el('lic-v2-close');
    const cancelButton = el('lic-v2-cancel');
    const packageButton = el('lic-v2-pkg-link');
    if (!body) return;
    if (prev) prev.disabled = _step <= 1 || !!_result || _generationInFlight;
    if (closeButton) closeButton.disabled = _generationInFlight;
    if (cancelButton) cancelButton.disabled = _generationInFlight;
    if (packageButton) packageButton.disabled = _generationInFlight;
    if (next) {
      next.disabled = _generationInFlight;
      if (_generationInFlight) next.textContent = '⏳ جارٍ التوليد...';
      else if (_step === 5 && !_result) next.textContent = '🔑 توليد الترخيص';
  ...
  CL.drawer = {
    ...
    close,
    injectRenewButtons,
    formatVaultActivationRow,
    formatVaultBundleRow,
    copyVaultActivationRow: (result) => copyText(formatVaultActivationRow(result), 'تم نسخ صف activations — الصق من A2'),
    copyVaultBundleRow: (result) => copyText(formatVaultBundleRow(result), 'تم نسخ bundle — الصق من A2')
  };
