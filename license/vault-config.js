...
(function (global) {
  ...
  async function saveOverride(webAppUrl, enabled) {
    ...
    const result = typeof global.persistData === 'function'
      ? await global.persistData('settings', global.settings)
      : await global.SqliteBridge?.setAuthoritative?.('settings', global.settings);
    if (!result?.ok) return { ok: false, error: result?.error || 'settings_commit_failed' };
    return getConfig();
  }
