(function (global) {
  ...
  const EXPIRY_SENTINEL = 0x1FFF;
  const MAX_EXPLICIT_EXPIRY_DAYS = EXPIRY_SENTINEL - 1;
  ...
  async function computeV5Mac(seg2, fields) {
    ...
    if (typeof CL?.crypto?.hmacTruncated !== 'function') {
      throw new Error('legacy_v5_signing_disabled_use_v6_migration');
    }
  ...
  async function encodeV5Key(opts) {
    ...
    const requestedExpiryDays = daysSinceEpoch(opts.expiry);
    if (!Number.isFinite(requestedExpiryDays) || requestedExpiryDays < 0) {
      throw new Error('v5_expiry_before_epoch');
    }
    const expiryOverflow = subNum === 8 || requestedExpiryDays > MAX_EXPLICIT_EXPIRY_DAYS;
    const expiryDays = expiryOverflow ? EXPIRY_SENTINEL : requestedExpiryDays;
    const fields = {
    ...
    return {
      key: formatKey25(raw),
      seg2, fields,
      expiry: subNum === 8 ? '2099-12-31' : (expiryOverflow ? String(opts.expiry) : daysToISO(expiryDays)),
      expiryOverflow,
      licenseSeq
    };
  }
  ...
  async function decodeV5Key(key) {
    ...
    try {
      ...
      const lifetime = fields.subscriptionId === 8;
      const expiryOverflow = fields.expiryDays === EXPIRY_SENTINEL;
      return {
        ok: true,
        seg2, fields, packageId: parsed.packageId, customPackageId: parsed.customPackageId,
        // Legacy lifetime keys used a wrapped expiry value. Subscription 08 is
        // therefore authoritative for every historical and newly issued key.
        expiry: lifetime ? '2099-12-31' : (expiryOverflow ? null : daysToISO(fields.expiryDays)),
        expiryOverflow,
  ...
  CL.codecV5 = {
    ...
    EXPIRY_SENTINEL, MAX_EXPLICIT_EXPIRY_DAYS
  };
