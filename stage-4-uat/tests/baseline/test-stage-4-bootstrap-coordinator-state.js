...
function readyEnv(overrides = {}) {
  ...
  const snap = {
    ...
    wizard: { path: 'new', currentStep: 0, restoreChoice: 'empty', syncDone: false, completedSteps: [] },
    settings: { centerName: 'S4 Center', backup: { providers: { google: { connected: true, oauth: true } } } },
    ...snapOverrides,
  };
...
(() => {
  const ctx = readyEnv({
    ...
    wizard: {
      ...
      syncDone: true, restoreChoice: 'empty',
    },
  });
...
(() => {
  const ctx = readyEnv({
    wizard: { path: 'new', currentStep: 0, completedSteps: [], syncDone: false, restoreChoice: null },
  });
...
(() => {
  const ctx = readyEnv({
    ...
    deviceConfig: { deviceUuid: '', lockedBranchId: '', deviceName: '' },
    wizard: { path: 'new', currentStep: 7, completedSteps: ['language', 'google', 'license', 'organization'], restoreChoice: 'empty' },
  });
  const resume = ctx.BootstrapCoordinator.resolveResumeStepIndex('new', 7);
  check(resume <= 4, 'resume snaps back from stale step 7 when branch/device missing');
})();
