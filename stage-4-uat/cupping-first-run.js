...
(function (global) {
  ...
  function ensureFirstRunSettings() {
    ...
    // BootFlow/SetupStateService is the sole automatic production setup state
    // machine. This authenticated helper remains available only when an admin
    // explicitly opens it from Settings.
    const unifiedSetupComplete = global.SetupStateService?.getState?.({ ignoreRestart: true })?.state === 'READY'
      || global.BootFlow?.isBootComplete?.() === true;
    if (unifiedSetupComplete && global.settings.firstRun.forceWizard !== true) {
      global.settings.firstRun.wizardCompleted = true;
      global.settings.firstRun.wizardSkipped = false;
    }
...
`;
  ...
  function saveWizardStepData() {
    ...
    if (step.id === 'center') {
      ...
      if (start > 0) global.invoiceCounter = start;
    } else if (step.id === 'devices') {
      if (typeof global.ensureDeviceSettings === 'function') global.ensureDeviceSettings();
  ...
  function skipSetupWizard() {
    ...
    saveFirstRunState({ wizardSkipped: true, wizardStep: _wizardStep, forceWizard: false });
    closeSetupWizard();
  ...
  function shouldShowSetupWizard() {
    ...
    ensureFirstRunSettings();
    if (global.SetupStateService?.evaluateReady?.({ ignoreRestart: true })?.ready === true) return false;
    // Never auto-open a second setup workflow after BootFlow. Explicit manual
    // invocation from Settings sets forceWizard and is the only entry.
    return global.settings.firstRun.forceWizard === true;
  }
