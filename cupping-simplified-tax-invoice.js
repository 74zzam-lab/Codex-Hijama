...
(function (global) {
  ...
  async function toggleInvoiceSystemUI(enabled) {
    ensureInvoiceSystemSettings();
    global.settings.invoiceSystem.enabled = !!enabled;
    const committed = await global.persistData?.('settings', global.settings);
    if (!committed || committed.ok === false) return false;
    applyInvoiceSystemVisibility();
    if (!enabled && typeof global.toggleSimplifiedTaxInvoiceUI === 'function') {
      await global.toggleSimplifiedTaxInvoiceUI(false);
  ...
  async function setVatCalcMode(mode) {
    ensureVatCalcSettings();
  ...
  function applySimplifiedTaxInvoiceVisibility(enabled) {
  ...
  async function toggleSimplifiedTaxInvoiceUI(enabled) {
    if (!isInvoiceSystemEnabled()) enabled = false;
    ensureSimplifiedTaxSettings();
    global.settings.simplifiedTaxInvoice.enabled = !!enabled;
    const committed = await global.persistData?.('settings', global.settings);
    if (!committed || committed.ok === false) return false;
    applySimplifiedTaxInvoiceVisibility(enabled);
    return true;
  }

  async function toggleTaxContactQrSetting(enabled) {
    ensureSimplifiedTaxSettings();
  ...
  async function saveZatcaSettings() {
    ...
    if (modeEl) await setVatCalcMode(modeEl.value);
  ...
  function loadSimplifiedTaxInvoiceUI() {
    ...
    applySimplifiedTaxInvoiceVisibility(!!global.settings.simplifiedTaxInvoice.enabled);
    bindZatcaQrPreview();
  }
