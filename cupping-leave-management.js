...
async function saveLeaveRequests() {
  const committed = await persistData('employeeLeaveRequests', employeeLeaveRequests);
  if (!committed?.ok) throw new Error(committed?.error || 'leave_requests_commit_failed');
  return committed;
}
...
async function syncApprovedLeaveToAttendance(req) {
  ...
  const committed = await persistData('attendance', attendance);
  if (!committed?.ok) throw new Error(committed?.error || 'leave_attendance_commit_failed');
  return committed;
}
...
async function setLeaveRequestStatus(id, status) {
  ...
  if (status === 'approved') await syncApprovedLeaveToAttendance(req);
...
async function saveLeavePolicySettings() {
  ...
  const committed = await persistData('settings', settings);
...
async function extRestoreLeaveData(data) {
